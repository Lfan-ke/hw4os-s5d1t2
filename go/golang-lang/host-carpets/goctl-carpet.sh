...
# (e.g. under bin/ or gopath/bin/). Defaults to a ./.go-toolchain dir beside this
# script; set it to your own layout on other hosts:
#   GO_TOOLCHAIN_ROOT=/path/to/go-toolchain bash goctl-carpet.sh
TC="${GO_TOOLCHAIN_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/.go-toolchain}"
export GOROOT="$TC/go"
