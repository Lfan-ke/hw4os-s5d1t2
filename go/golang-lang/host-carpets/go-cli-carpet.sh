...
#
# GROUND TRUTH (enumerated item-by-item from the docs below):
...
# recognized command via help; do NOT invoke it (no display, headless environment).
assert_has "bug/is-command" "bug" "$(run_out "$GO" help bug)"
skip "bug/invoke" "go bug opens a browser to file an issue (display/interactive) — headless env"
