// KotlinJsCarpet.js — carpet for the Kotlin/JS delivery on Node.js v22.22.2 (StarryOS target runtime).
//
// Runs the host-precompiled Kotlin->JS module (kotlin-app.js, emitted by the Kotlin 2.0.21 IR
// backend with -module-kind commonjs) as a child process, captures its stdout/stderr/exit, and
// asserts the output is byte-identical to the golden kotlin-REF.out with thorough per-line and
// per-token checks. The module self-executes its `main` on load (writes 6 lines via console.log).
//
...
// v22.22.2 binary. Marker: KOTLINJS_DONE printed ONLY when fail===0.
...
chk(process.version === 'v22.22.2', 'node version is v22.22.2 (got ' + process.version + ')');
...
chk(appStat.size === 657816, 'kotlin-app.js size === 657816 bytes exactly (got ' + appStat.size + ')');
