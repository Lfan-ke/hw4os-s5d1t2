#!/usr/bin/env bash
# prebuild.sh — provision the node-web framework carpet for StarryOS.
#
# A set of real Node.js web frameworks — pug (template engine), express (web framework, driven
# over IPv4 loopback) and Kotlin/JS (a host-precompiled Kotlin->JS module run on node) — each
# run on-target by Node.js v22.22.2 (full V8 JIT, no kernel change) with an anchored self-check.
#
# It stages the musl-native Node.js v22.22.2 apk closure into the per-app rootfs /usr, plus the
# arch-independent carpet sources + the pug/express node_modules closure + the Kotlin/JS module,
# then the harness injects the overlay and qemu-<arch>.toml runs /usr/bin/run-nweb.sh.
#
...
# Stage the carpet sources + pug/express node_modules + Kotlin/JS module into /root/nweb and
# the run-nweb.sh gate into /usr/bin.
stage_payload() {
    local nw="$overlay_dir/root/nweb"
    install -d "$nw/carpets"
    install -m0644 "$PROG/carpets/PugCarpet.js"      "$nw/carpets/"
    install -m0644 "$PROG/carpets/ExpressCarpet.js"  "$nw/carpets/"
    install -m0644 "$PROG/carpets/KotlinJsCarpet.js" "$nw/carpets/"
    cp -a "$ASSETS/node_modules" "$nw/node_modules"
    # kotlin module + golden live next to KotlinJsCarpet.js (it resolves them via __dirname).
    install -m0644 "$ASSETS/kotlin-app.js"  "$nw/carpets/kotlin-app.js"
    install -m0644 "$ASSETS/kotlin-REF.out" "$nw/carpets/kotlin-REF.out"
    install -Dm0755 "$PROG/run-nweb.sh" "$overlay_dir/usr/bin/run-nweb.sh"
    echo "prebuild: staged carpets + node_modules ($(du -sh "$nw/node_modules" | cut -f1)) into /root/nweb, run-nweb.sh into /usr/bin"
}
...
main() {
    ...
    stage_node
    stage_payload
    echo "prebuild: node-web overlay ready for $arch — $(du -sh "$overlay_dir" | cut -f1)"
}
