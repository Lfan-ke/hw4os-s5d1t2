...
# The jdk25u source (tag jdk-25.0.4+5) guards the lui->c.lui peephole with imm!=0 (the only
# caller of c_lui), so a server VM built from it never emits that instruction. We cross-compile
...
DL="${JAVA_DL_ROOT:?set JAVA_DL_ROOT to your download cache dir}"
JM="$DL/jdk-multi"
SRC="$DL/rv-jdk25-src"
