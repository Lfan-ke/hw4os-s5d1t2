...
The cells are byte-for-byte the **PROVEN, 4-arch-green `openjdk-multi`** delivery
...
| :--: | :--: | :--: | :--: | :--: | :--: |
| x86_64 | apk (openjdk17) | BellSoft musl | BellSoft musl | BellSoft musl | 4/4 |
...
**JDK23 N/A on riscv64 / loongarch64** (documented per the partial-arch-tick rule):
upstream ships JDK23 for these arches only as glibc, and **glibc + gcompat segfaults
the JVM — verified on REAL Linux 6.18 too, so this is NOT a StarryOS limitation**.
Those two arches therefore run **3 JDKs (17/21/25)** with a **3/3** version switch.
x86_64 / aarch64 run the **full 4 JDKs** with a **4/4** switch. `javac`, `java`, the
language/grammar/CLI carpets, and BackCompat run on every staged JDK on every arch.
...
1. **Belt-and-suspenders force-baseline.** On riscv64 it passes the JVM
   extension-disable flags with the correct per-JDK unlock token
   (`-XX:+UnlockExperimentalVMOptions` for JDK≤21, `-XX:+UnlockDiagnosticVMOptions`
   for JDK≥24) → `-XX:-UseRVV -XX:-UseZba -XX:-UseZbb -XX:-UseZbs`, forcing the JVM
   back to RV64GC so it cannot emit `Zb*`/vector in its generated stubs. On the
   baseline these flags are already off (harmless).
2. **Documented SKIP, never a false failure.** Each JDK is liveness-probed
   ...
   fails the run fails fast. On `x86_64 / aarch64` no JDK is skipped.
...
set at run time, so one script serves all 4 arches.
...
```bash
source <tgoskits 仓库根>/.starry-env.sh   # qemu-10
...
> is validated through CI like the other prebuild apps. aarch64 / riscv64 /
> loongarch64 boot locally.
