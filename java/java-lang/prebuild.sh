...
#
# ── ROOTFS SIZE (the crucial lesson from the kotlin-lang case) ───────────────────────
...
# them can populate the cache (or point BCREAL_LOCAL at a prebuilt-artifact tree, see
# below). sha256 are the host-verified, 299-test-green delivered copies.
BCREAL_LIBS=(
...
# Local prebuilt fallback root (the host-built, 299-test-green artifacts). Override
# BCREAL_LOCAL to a local prebuilt tree; defaults to a repo/staging-relative cache so a
# fresh checkout has a writable place and no machine-specific path is assumed.
BCREAL_LOCAL="${BCREAL_LOCAL:-${STARRY_STAGING_ROOT:-$app_dir}/.cache/java-backcompat-real}"
