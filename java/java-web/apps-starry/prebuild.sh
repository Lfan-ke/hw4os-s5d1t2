...
#
# A set of real JEE/JVM frameworks — Jetty / Netty (loopback HTTP servers), MyBatis / Hibernate
# (ORM over an in-memory DB), R2DBC (reactive DB) and a real .war deployment into an embedded
# Jetty servlet container — each run on-target by OpenJDK 17 with an anchored self-check marker.
#
# It stages ONE full JDK17 (the openjdk-multi JDK17 cell, proven 4-arch green) plus the
# arch-independent framework carpet jars into the per-app rootfs, then the harness injects
# the overlay and qemu-<arch>.toml runs /usr/bin/run-jweb.sh.
#
...
# Stage the JEE framework carpet jars into /root/jweb and the run-jweb.sh gate into /usr/bin.
stage_payload() {
    local jw="$overlay_dir/root/jweb"
    install -d "$jw" "$jw/native"
    install -m0644 "$ASSETS/jetty-demo.jar"     "$jw/"
    install -m0644 "$ASSETS/netty-demo.jar"     "$jw/"
    install -m0644 "$ASSETS/mybatis-demo.jar"   "$jw/"
    install -m0644 "$ASSETS/hibernate-demo.jar" "$jw/"
    install -m0644 "$ASSETS/r2dbc-demo.jar"     "$jw/"
    # MyBatis/Hibernate use sqlite-jdbc; its jar bundles musl native only for x86_64/aarch64.
    # riscv64/loongarch64 load the cross-built JNI .so (run-jweb.sh points org.sqlite.lib.path at it).
    case "$arch" in
        riscv64)     install -m0644 "$ASSETS/native/libsqlitejdbc-riscv64.so"     "$jw/native/libsqlitejdbc.so" ;;
        loongarch64) install -m0644 "$ASSETS/native/libsqlitejdbc-loongarch64.so" "$jw/native/libsqlitejdbc.so" ;;
    esac
    install -Dm0755 "$PROG/run-jweb.sh" "$overlay_dir/usr/bin/run-jweb.sh"
    echo "prebuild: staged JEE framework jars into /root/jweb, run-jweb.sh into /usr/bin"
}
