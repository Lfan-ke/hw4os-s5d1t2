/* S11 · 简易协议栈实现（参考解：构造给定；IP/UDP 校验和与 UDP 解析为填空点）。
 *
...
uint16_t udp_checksum(const uint8_t src[IPV4_ALEN], const uint8_t dst[IPV4_ALEN],
    ...
    uint8_t pseudo[12];
    kmemcpy(pseudo + 0, src, IPV4_ALEN);
    kmemcpy(pseudo + 4, dst, IPV4_ALEN);
    pseudo[8]  = 0;
    pseudo[9]  = IPPROTO_UDP;
    pseudo[10] = udp_len >> 8; pseudo[11] = udp_len & 0xFF;

    uint32_t sum = cksum_accumulate(pseudo, 12, 0);
    sum = cksum_accumulate(udp_seg, udp_len, sum);
...
int udp_parse(const uint8_t *ipframe, uint32_t framelen, struct udp_recv *out) {
    if (framelen < IP_HDR_LEN) return -1;
    uint32_t ihl = (uint32_t)(ipframe[0] & 0x0F) * 4;
    if (ihl < IP_HDR_LEN || framelen < ihl + UDP_HDR_LEN) return -1;
    if (ipframe[9] != IPPROTO_UDP) return -1;

    /* IP 头校验和：合法时全头反码和折叠为 0 */
    if (cksum_fold(cksum_accumulate(ipframe, ihl, 0)) != 0) return -1;

    const uint8_t *udp = ipframe + ihl;
    uint16_t udp_len = (uint16_t)(((uint32_t)udp[4] << 8) | udp[5]);
    if (udp_len < UDP_HDR_LEN || framelen < ihl + udp_len) return -1;

    /* UDP 伪首部 + 段校验和：合法时折叠为 0 */
    uint8_t pseudo[12];
    kmemcpy(pseudo + 0, ipframe + 12, IPV4_ALEN);
    kmemcpy(pseudo + 4, ipframe + 16, IPV4_ALEN);
    pseudo[8]  = 0;
    pseudo[9]  = IPPROTO_UDP;
    pseudo[10] = udp_len >> 8; pseudo[11] = udp_len & 0xFF;
    uint32_t sum = cksum_accumulate(pseudo, 12, 0);
    sum = cksum_accumulate(udp, udp_len, sum);
    if (cksum_fold(sum) != 0) return -1;

    kmemcpy(out->src_ip, ipframe + 12, IPV4_ALEN);
    kmemcpy(out->dst_ip, ipframe + 16, IPV4_ALEN);
    out->sport   = (uint16_t)(((uint32_t)udp[0] << 8) | udp[1]);
    out->dport   = (uint16_t)(((uint32_t)udp[2] << 8) | udp[3]);
    out->payload = udp + UDP_HDR_LEN;
    out->paylen  = (uint32_t)(udp_len - UDP_HDR_LEN);
    return 0;
}
