...
static BoardConfig bsp_probe(uint32_t board_id) {
    BoardConfig c;
    switch (board_id) {
        case 0:  c.uart_base = A_UART_BASE; c.clk_hz = A_CLK_HZ; break;
        case 1:  c.uart_base = B_UART_BASE; c.clk_hz = B_CLK_HZ; break;
        default: c.uart_base = 0;           c.clk_hz = 0;        break;
    }
    return c;
}
...
static BoardConfig parse_dt(const Dtb *blob) {
    BoardConfig c; c.uart_base = 0; c.clk_hz = 0;
    for (int i = 0; i < blob->n; i++) {
        const DtNode *nd = &blob->nodes[i];
        if (strcmp(nd->compatible, "vlab,uart") == 0) {
            c.uart_base = nd->reg_base;
            for (int j = 0; j < nd->nprop; j++)
                if (strcmp(nd->props[j].name, "clock-frequency") == 0) c.clk_hz = nd->props[j].val;
        }
    }
    return c;
}
...
static int driver_bind(const Dtb *blob, DriverRec *regs, int nreg) {
    ...
    for (int i = 0; i < blob->n; i++) {
        for (int k = 0; k < nreg; k++) {
            if (strcmp(regs[k].compatible, blob->nodes[i].compatible) == 0) {
                /* regs[k].probe(reg_base, irq) —— 真实内核在此建设备实例 */
                regs[k].bound++; total++; break;
            }
        }
    }
    return total;
...
static BoardConfig parse_dt_v2(const Dtb *blob) {
    BoardConfig c; c.uart_base = 0; c.clk_hz = DEFAULT_CLK_HZ;
    for (int i = 0; i < blob->n; i++) {
        const DtNode *nd = &blob->nodes[i];
        if (strcmp(nd->compatible, "vlab,uart") == 0) {
            c.uart_base = nd->reg_base;
