...
//! 学生只填四个函数体；下方测试 harness（MMIO 总线模型 + PASS 打印）勿改。
#![allow(dead_code)]
...
fn bsp_probe(board_id: u32) -> BoardConfig {
    // 参考解：TODO[a] 静态 match 表（板 0→A 地址，板 1→B 地址）。
    match board_id {
        0 => BoardConfig { uart_base: A_UART_BASE, clk_hz: A_CLK_HZ },
        1 => BoardConfig { uart_base: B_UART_BASE, clk_hz: B_CLK_HZ },
        _ => BoardConfig { uart_base: 0, clk_hz: 0 },
    }
}

/// 2) 用设备树替代硬编码：找 compatible=="vlab,uart" 的节点取 reg/clk。
fn parse_dt(blob: &Dtb) -> BoardConfig {
    // 参考解：TODO[a] 顺序扫描匹配 compatible。
    let mut cfg = BoardConfig { uart_base: 0, clk_hz: 0 };
    for node in blob {
        if node.compatible == "vlab,uart" {
            cfg.uart_base = node.reg_base;
            for p in &node.props {
                if p.name == "clock-frequency" {
                    cfg.clk_hz = p.val;
                }
            }
        }
...
fn driver_bind(blob: &Dtb, regs: &mut [DriverRec]) -> u32 {
    // 参考解：线性匹配；命中即 probe（这里只统计），返回总绑定数。
    let mut bound_total = 0;
    for node in blob {
        for drv in regs.iter_mut() {
            if drv.compatible == node.compatible {
                // drv.probe(node.reg_base, node.irq) —— 真实内核在此建设备实例
                drv.bound += 1;
                bound_total += 1;
                break;
            }
...
fn parse_dt_v2(blob: &Dtb) -> BoardConfig {
    // 参考解：TODO[a] 缺省值兜底；未知属性走 _ 分支跳过。
    let mut cfg = BoardConfig { uart_base: 0, clk_hz: DEFAULT_CLK_HZ };
    for node in blob {
        if node.compatible == "vlab,uart" {
            cfg.uart_base = node.reg_base;
            for p in &node.props {
                match p.name {
                    "clock-frequency" => cfg.clk_hz = p.val,
                    _ => { /* 未知属性：跳过，不报错 */ }
                }
            }
        }
