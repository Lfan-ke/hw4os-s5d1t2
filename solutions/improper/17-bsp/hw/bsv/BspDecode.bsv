// 板级地址译码器 —— 硬件路径（BlueSpec SV 参考解）。
// sel  = 地址落在 [base, base+size) 窗口内；rdata = 命中时设备应答 magic|offset。
// 与软件 bsp_probe / Verilog bsp_decode 完全同构。
package BspDecode;

// ── 核心逻辑：填这个函数体 ────────────────────────────────────────
function Tuple2#(Bit#(1), Bit#(32)) bsp_decode(Bit#(32) base, Bit#(32) size, Bit#(32) addr);
   Bit#(32) magic = 32'hDEC0_0000;
   Bit#(1)  sel   = ((addr >= base) && (addr < (base + size))) ? 1'b1 : 1'b0;
   Bit#(32) rdata = (sel == 1) ? (magic | (addr - base)) : 32'h0000_0000;
