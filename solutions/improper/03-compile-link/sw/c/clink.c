...
/* 向上对齐到 a（a 为 2 的幂）。链接脚本的 `. = ALIGN(a);`。 */
...
static void layout(uint64_t base, const uint64_t sizes[N_SEC], uint64_t starts[N_SEC]) {
    ...
    starts[0] = base;
    for (int i = 1; i < N_SEC; i++) starts[i] = starts[i - 1] + sizes[i - 1];
}
...
static int parse_elf(const uint8_t *buf, int n, uint64_t *entry, uint64_t *phoff) {
    ...
    int magic_ok = (n >= 64) && buf[0] == 0x7F && buf[1] == 0x45 && buf[2] == 0x4C && buf[3] == 0x46;
    uint64_t e = 0, p = 0;
    for (int i = 0; i < 8; i++) e |= (uint64_t)buf[24 + i] << (8 * i);
    for (int i = 0; i < 8; i++) p |= (uint64_t)buf[32 + i] << (8 * i);
    *entry = e; *phoff = p;
    return magic_ok;
}
