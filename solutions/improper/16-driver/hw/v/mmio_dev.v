...
module mmio_dev (
    ...
    always @(*) begin
        case (addr)
            R_ID:     rdata = MAGIC;
            R_CTRL:   rdata = {31'b0, enabled};
            R_STATUS: rdata = {31'b0, ready};
            R_DATA:   rdata = {24'b0, last};
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            case (addr)
                R_CTRL: enabled <= wdata[0];
                R_DATA: if (ready) last <= wdata[7:0]; // 未就绪写被忽略
                default: ; // ID/STATUS 只读
            endcase
        end
    end
endmodule
