 ...
 *   FIFO → 约束调度（GHOST 永不上、B 恒在 A 后、其余任意）→ 优先级。
 * 学生只填三个函数体；下方测试 harness 勿改。
 */
#include <stdio.h>
#include <stdint.h>
...
static int run_fifo(const Proc *p, int n, uint32_t *out) {
    for (int i = 0; i < n; i++) out[i] = p[i].pid;
    return n;
}
...
static int run_constrained(const Proc *p, int n, uint32_t *out) {
    int m = 0;
    for (int i = 0; i < n; i++)
        if (p[i].tag != GHOST) out[m++] = p[i].pid;
    /* 找 A、B 的 pid */
    uint32_t a = 0, b = 0; int hasA = 0, hasB = 0;
    for (int i = 0; i < n; i++) {
        if (p[i].tag == TAG_A) { a = p[i].pid; hasA = 1; }
        if (p[i].tag == TAG_B) { b = p[i].pid; hasB = 1; }
    }
    if (hasA && hasB) {
        int pa = -1, pb = -1;
        for (int i = 0; i < m; i++) { if (out[i] == a) pa = i; if (out[i] == b) pb = i; }
        if (pb < pa) {
            /* 删除 B，再插到 A 之后 */
            for (int i = pb; i < m - 1; i++) out[i] = out[i + 1];
            m--;
            for (int i = 0; i < m; i++) if (out[i] == a) { pa = i; break; }
            for (int i = m; i > pa + 1; i--) out[i] = out[i - 1];
            out[pa + 1] = b; m++;
        }
    }
    return m;
...
static int run_priority(const Proc *p, int n, uint32_t *out) {
    int idx[MAXP];
    /* 稳定插入排序：prio 降序，平级保持到达顺序 */
    for (int i = 0; i < n; i++) idx[i] = i;
    for (int i = 1; i < n; i++) {
        int key = idx[i], j = i - 1;
        while (j >= 0 && p[idx[j]].prio < p[key].prio) { idx[j + 1] = idx[j]; j--; }
        idx[j + 1] = key;
    }
    for (int i = 0; i < n; i++) out[i] = p[idx[i]].pid;
    return n;
}
