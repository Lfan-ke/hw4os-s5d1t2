...
static void step(uint32_t csr, uint32_t kind, uint32_t arg_priv, uint32_t arg_en,
    ...
    switch (kind) {
    /* 子实验 1：特权比较器——“有没有权限”就是一根 cur < need 的线。 */
    case NORMAL:
        trap = (cur < arg_priv);
        break;
    ...
    case DROP:
        if (arg_priv > cur) trap = 1;       /* 不许直接提权 */
        else                ncur = arg_priv; /* 合法下放 */
        break;

    /* 子实验 3：陷入提权 + 返回。 */
    case ECALL:
        nsp = cur; /* saved_priv ← cur_priv（同 MPP/SPP） */
        ncur = A;  /* 进入最高态处理 */
        break;
    case XRET:
        if (cur != A) trap = 1; /* 只有最高态能 xret */
        else          ncur = sp;
        break;

    /* 子实验 4：开启功能也是置位。能力 = 特权够 且 使能位亮。 */
    case SETFEAT:
        if (cur < B) trap = 1;     /* 配置使能位至少要 B 态 */
        else         nfe = arg_en;
        break;
    case USEFEAT:
        trap = (cur < arg_priv) || (fe == 0); /* 缺一不可 */
        break;
