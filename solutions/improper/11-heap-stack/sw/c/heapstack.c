/* 11 堆与栈：SP 是一根指针，allocator 是一个记账员 —— C 参考解。
 ...
 * 学生填各结构体函数 + 11.4 的「注册」一行；下方 harness（勿改）打印 *_PASS。
 */
#include <stdio.h>
#include <stdint.h>
#include <stddef.h>
...
/* 学生填：把 SP 初始化到内存区「顶端」（向下生长，故初值 = top）。 */
static void sp_init(Stack *s) {
    s->sp = s->top;
}

/* 学生填：满递减栈 push = 先 sp-=1 再写；到 base 仍压 → 越界（返回 -1）。 */
static int stack_push(Stack *s, uint32_t w) {
    if (s->sp <= s->base) return -1; /* STACK_OVERFLOW */
    s->sp -= 1;
    s->mem[s->sp] = w;
    return 0;
}

/* 学生填：pop = 先读 mem[sp] 再 sp+=1；空栈返回 -1，值经 *out 带回。 */
static int stack_pop(Stack *s, uint32_t *out) {
    if (s->sp >= s->top) return -1;
    *out = s->mem[s->sp];
    s->sp += 1;
    return 0;
}
...
/* 学生填：bump 向上分配 n 个字，越上限 → OOM（返回 -1）；起始下标经 *off 带回。 */
static int heap_alloc(HeapDev *h, size_t n, size_t *off) {
    if (h->top + n > h->cap) return -1; /* OOM */
    *off = h->top;
    h->top += n;
    return 0;
}
...
static int exp_indep(void) {
    ...
    Stack a; stack_new(&a); sp_init(&a);        /* 设备 A：小，作栈 */
    HeapDev b; heap_new(&b, 16); heap_init(&b); /* 设备 B：大，作堆 */
...
/* 学生填：堆 alloc 自查 heap_top + n <= sp，否则 OOM（无独立设备兜底，碰撞全靠手算）。 */
static int onedev_alloc(OneDev *m, size_t n, size_t *off) {
    if (m->heap_top + n > m->sp) return -1; /* OOM */
    *off = m->heap_top;
    m->heap_top += n;
    return 0;
}

/* 学生填：栈 push 自查 sp - 1 >= heap_top，否则 STACK_OVERFLOW。 */
static int onedev_push(OneDev *m, uint32_t w) {
    if (m->sp <= m->heap_top) return -1; /* STACK_OVERFLOW */
    m->sp -= 1;
    m->mem[m->sp] = w;
    return 0;
}
...
/* malloc/free 经全局指针 g_alloc 派发（这一段给好；学生的活是「注册」g_alloc）。 */
...
static int exp_global(void) {
    /* 学生填（11.4 的核心一行）：注册——把你的 allocator 接到全局指针 g_alloc。
