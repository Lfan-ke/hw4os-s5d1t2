//! 外核（Exokernel）形态认知 demo —— 学生填空版（Rust）。
//!
...
//!
//! 你只需填两处（标 TODO）：① exo_alloc 边界/重叠校验；② libb_place 分块布局。
...
fn exo_alloc(owner: &mut [i32; NBLK], who: i32, start: usize, len: usize) -> bool {
    // TODO: ① 越界校验——用 `len > NBLK - start` 比较（别写 start+len，会溢出）；
    //          再判 len==0 / start>=NBLK 也要拒绝。任一不满足 return false。
    // TODO: ② 重叠校验——遍历 start..start+len，只要有块 owner[b] != FREE 就 return false。
    // TODO: ③ 两关都过，再遍历 start..start+len 把 owner[b] = who，最后 return true。
    let _ = (owner, who, start, len);
    false // ← 占位：恒拒绝，过不了 EXO_PASS
}
...
/// 返回挑中的块号列表（逻辑顺序 = 采集顺序）。这与 libA「顺序升序」形成对比。
/// 块不够则返回 None（并保持 used 不变）。
fn libb_place(base: usize, len: usize, used: &mut [bool], n: usize) -> Option<Vec<usize>> {
    // TODO: ① 先数领地内空闲块数 avail；若 n==0 或 avail < n，return None（别留脏标记）。
    // TODO: ② 从 b = base+len-1 向 base 递减扫描，遇空闲块就 used[b]=true 并 push(b)，
    //          直到凑够 n 个；返回 Some(blocks)。（高块号在前——这就是与 libA 的区别。）
    let _ = (base, len, used, n);
    None // ← 占位：不放置，过不了 LIBOS_PASS
}
