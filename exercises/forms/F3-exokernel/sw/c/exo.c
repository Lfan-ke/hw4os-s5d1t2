 ...
 * 裁判规则（仅校验，不解释用途）：
 *   - 越界：len>0 且 start+len <= NBLK，否则拒绝。
 *   - 重叠：区间内每个块都必须空闲(owner==FREE)，否则拒绝。
 * 通过则标记归属并返回 1；否则返回 0 且不改动。 */
static int exo_alloc(int owner[NBLK], int who, int start, int len) {
    /* TODO: ① 越界校验——len<=0 / start<0 / start>=NBLK / len > NBLK-start 任一成立 return 0。
     *          （用 len > NBLK - start 比较，别写 start+len 防溢出。）
     * TODO: ② 重叠校验——遍历 start..start+len，只要有块 owner[b] != FREE 就 return 0。
     * TODO: ③ 两关都过，再遍历把 owner[b] = who，最后 return 1。 */
    (void)owner; (void)who; (void)start; (void)len;
    return 0; /* 占位：恒拒绝，过不了 EXO_PASS */
}
 ...
 * 与 libA「顺序升序」形成对比。块不够则返回 0 不改 used。 */
static int libb_place(int base, int len, int used[NBLK], int n, int out[]) {
    /* TODO: ① 先数领地内空闲块数 avail；若 n<=0 或 avail<n，return 0（别留脏标记）。
     * TODO: ② 从 b = base+len-1 向 base 递减扫描，遇空闲块就 used[b]=1、out[cnt++]=b，
     *          直到凑够 n 个；返回 cnt。（高块号在前——这就是与 libA 的区别。） */
    (void)base; (void)len; (void)used; (void)n; (void)out;
    return 0; /* 占位：不放置，过不了 LIBOS_PASS */
}
