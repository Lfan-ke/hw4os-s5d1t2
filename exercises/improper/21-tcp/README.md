...
## 6. 引申：从这台「裸状态机」长成真正的 TCP

本课为了把「连接 = 状态 + 序号」这件事看清，砍掉了真实 TCP 的几乎所有可靠性机制：
mock 通道**不丢不乱不重复**、ISN **写死**、没有重传定时器、没有窗口、没有校验和。
真实 TCP 的复杂度恰恰全在这些被砍掉的地方。按兴趣选一个方向往下挖：

1. **把 mock 通道变得「不可靠」，逼出重传**：给通道加丢包/乱序/重复，然后实现
   RTO 重传定时器（`snd_una` 游标 + 未确认队列）、**快速重传**（3 个重复 ACK 即重发），
   再加 RTT 估计（Jacobson/Karn 算法算 SRTT/RTO）。这是 TCP「可靠」二字的真正引擎。
2. **加滑动窗口与流量控制**：报文补 `window` 字段，发送方维护 `snd_una/snd_nxt/snd_wnd`，
   只发「窗口内」的字节；接收方用 `rwnd` 反压。顺手做 **Nagle**（攒小包）与**延迟 ACK**，
   体会它俩凑一块儿为何会互相拖出延迟。
3. **加拥塞控制**：在流量控制之外再叠一层 `cwnd`，实现慢启动→拥塞避免→快速恢复
   （TCP Reno）。有余力对照 **CUBIC**（Linux 默认）与 **BBR**（Google，基于带宽×时延而非丢包）。
4. **随机 ISN + 失序重排缓冲**：把 `ISS_A/ISS_B` 换成随机（甚至 RFC 6528 的加密散列），
   再给接收端加一个**重排缓冲区**接住失序段、配 **SACK** 选择性确认——这才撑得起真实乱序网络。
5. **接真实协议栈**：把这台状态机塞进 IP/以太网帧里跑。可参考 **smoltcp**（Rust，no_std，
   rCore/嵌入式都在用）或 **lwIP**（C），用 tun/tap 设备和真机/QEMU 对端做一次真实三次握手。
6. **对照真实实现读源码**：Linux `net/ipv4/tcp_input.c`（收包状态机）、`tcp_output.c`、
   xv6 没有 TCP 而 rCore-Tutorial 用 smoltcp——对比「教学状态机」与生产状态机在
   `TIME_WAIT` 复用、半关闭、RST 处理上的差距，正好回扣 `proper/S11` 的 UDP-only 现状。

## 7. 思考题（`essay/THINKING.md` 作答即可通过）
