    ...
    fn recv(&mut self, s: Seg) -> Seg {
        ...
        match self.state {
            ...
            State::Listen if has(s.flags, SYN) && !has(s.flags, ACK) => {
                ...
                //    用 Seg::new(SYN | ACK, self.iss, self.rcv_nxt, 0) 构造。
                // 3) 自己的 snd_nxt = self.iss + 1（我的 SYN 也占 1 个序号）。
                // 4) self.state = State::SynRcvd，返回那个 SYN|ACK 段。
            ...
            State::SynSent if has(s.flags, SYN) && has(s.flags, ACK) => {
                ...
                // 2) 回一个纯 ACK：Seg::new(ACK, self.snd_nxt, self.rcv_nxt, 0)。
                // 3) self.state = State::Established，返回那个 ACK 段。
                // 当前占位：原样不动、不回报文 → 握手起不来（HANDSHAKE_MISS）。
                let _ = &s;
                Seg::none()
                // ────────────────────────────────────────────────────────────
