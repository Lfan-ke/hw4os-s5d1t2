...
static Seg tcp_recv(Tcp *t, Seg s) {
    ...
    if (t->state == LISTEN && has(s.flags, SYN) && !has(s.flags, ACK)) {
         ...
         * 1) t->rcv_nxt = s.seq + 1（SYN 占 1 个序号）。
         * 2) 回一个 SYN|ACK：seg_new(SYN | ACK, t->iss, t->rcv_nxt, 0)。
         * 3) t->snd_nxt = t->iss + 1（我的 SYN 也占 1 个序号）。
         * 4) t->state = SYN_RCVD，返回那个 SYN|ACK 段。
         * 当前占位：原样不动、不回报文 → 握手起不来（HANDSHAKE_MISS）。 */
        return seg_none();
        /* ──────────────────────────────────────────────────── */
    }
    ...
    if (t->state == SYN_SENT && has(s.flags, SYN) && has(s.flags, ACK)) {
         ...
         * 1) t->rcv_nxt = s.seq + 1。
         * 2) 回纯 ACK：seg_new(ACK, t->snd_nxt, t->rcv_nxt, 0)。
         * 3) t->state = ESTABLISHED，返回那个 ACK 段。
         * 当前占位：原样不动、不回报文 → 握手起不来（HANDSHAKE_MISS）。 */
        return seg_none();
        /* ──────────────────────────────────────────────────── */
    }
    ...
    if (t->state == ESTABLISHED && s.len > 0) {
        /* ───────────── TODO ③ 数据 seq/ack 推进 ─────────────
         * t->rcv_nxt = s.seq + s.len；回纯 ACK：seg_new(ACK, t->snd_nxt, t->rcv_nxt, 0)。
         * 当前占位：不推进、不回 ACK → DATA_MISS / DATA_BAD。 */
        return seg_none();
        /* ──────────────────────────────────────────────────── */
    }
