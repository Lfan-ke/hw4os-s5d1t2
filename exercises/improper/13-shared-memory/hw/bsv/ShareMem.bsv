// 设备↔OS 的 MMIO 共享环 mailbox —— 硬件路径（BlueSpec SV）。
// 定深=4 的 ring 写成纯函数（状态 struct 经 let 串起），与软件 Ring / Verilog
// ring_mbox 完全同构。你只需填 ring_push / ring_pop。
package ShareMem;
...
// HINT:
//   if (ring_full(r)) return tuple2(r, False);
//   else begin Ring n=r; n.buff=update(r.buff,r.tail,x); n.tail=r.tail+2'd1; n.cnt=r.cnt+3'd1;
//              return tuple2(n, True); end
function Tuple2#(Ring, Bool) ring_push(Ring r, Bit#(32) x);
   // ← 占位：永远入队失败（touch x 仅为消除未用告警）→ RING_FAIL
   Ring n = r;
   n.cnt = r.cnt ^ truncate(x & 32'd0);
   return tuple2(n, False);
endfunction

// 出队：空则 (r, False, 0)；否则读 head、抬 head、cnt-1，返回 (新环, True, 队首)。
// HINT:
//   if (!ring_avail(r)) return tuple3(r, False, 32'd0);
//   else begin Ring n=r; Bit#(32) v=select(r.buff,r.head); n.head=r.head+2'd1; n.cnt=r.cnt-3'd1;
//              return tuple3(n, True, v); end
function Tuple3#(Ring, Bool, Bit#(32)) ring_pop(Ring r);
   // ← 占位：永远出队失败（select 仅为消除未用告警）→ RING_FAIL
   return tuple3(r, False, select(r.buff, r.head) & 32'd0);
endfunction
