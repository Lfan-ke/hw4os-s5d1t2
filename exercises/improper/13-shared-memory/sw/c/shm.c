 ...
 *   同一份物理字节 + 一套不踩脚的协调协议。
 *
 * 你只需填下方 6 处标了「TODO」的地方；harness 勿改。
 ...
 * HINT: pt[vpn].valid = 1; pt[vpn].ppn = ppn; */
static void map(Pte *pt, int vpn, int ppn) {
    (void)pt; (void)vpn; (void)ppn;
    /* ← 占位：没建立映射，translate 将返回 -1 → ALIAS_FAIL */
}
...
static int sub_alias(void) {
     ...
     * HINT: map(pt, vpn_a, ppn_shared); map(pt, vpn_b, ppn_shared); */
    (void)ppn_shared;

    unsigned va_a = vpn_a * PAGE_WORDS + 1;
...
static void do_mmap(World *w, Pte *pt, int vpn, uint32_t key, uint32_t flags) {
    (void)w; (void)pt; (void)vpn; (void)key; (void)flags; (void)frame_alloc;
    /* TODO[a] MAP_SHARED：遍历 w->keys 找 key，命中复用 w->ppns[i]；
     *   未命中再 frame_alloc(w) 并登记到 keys/ppns/nreg。
     * ELSE[b] MAP_PRIVATE：直接 frame_alloc(w) 一块新（零）帧。
     * 求出 ppn 后：map(pt, vpn, ppn);
     * ← 占位：没建立映射 → translate -1 → SHARED_FAIL */
}
...
/* TODO(4): 生产者一步——先写满 data，最后才置 ready。
 * HINT: for(i..) mb->data[i]=payload[i];  然后  mb->ready=1; */
static void producer_step(Mailbox *mb, const uint32_t *payload) {
    (void)mb; (void)payload;
    /* ← 占位：没写 data、没置 ready → MAILBOX_FAIL */
}

/* TODO(5): 消费者一步——仅当 ready 才拷贝；拷贝后清 ready。
 * 返回 1 表示读到（out 已填），0 表示无数据。
 * HINT: if(!mb->ready) return 0; for(i..) out[i]=mb->data[i]; mb->ready=0; return 1; */
...
/* TODO(6a): 入队（生产侧）。满则返回 0；否则写 tail、抬 tail、count+1。
 * HINT: if(r->count==CAP) return 0; r->buf[r->tail]=x; r->tail=(r->tail+1)%CAP; r->count++; return 1; */
static int ring_push(Ring *r, uint32_t x) {
    (void)r; (void)x;
    return 0; /* ← 占位：永远入队失败 → RING_FAIL */
}

/* TODO(6b): 出队（消费侧）。空则返回 0；否则经 *out 给出 head 元素、抬 head、count-1。
 * HINT: if(r->count==0) return 0; *out=r->buf[r->head]; r->head=(r->head+1)%CAP; r->count--; return 1; */
