...
//!   同一份物理字节 + 一套不踩脚的协调协议。
//!
//! 你只需填下方 6 处标了「TODO」的地方；harness（向量 + 校验 + PASS 打印）勿改。
//! 通过段会逐段打印 *_PASS，全过再打印 ALL_PASS。
...
/// TODO(1): 把虚拟页 vpn 映射到物理页 ppn（写一个有效 PTE）。
/// HINT: pt[vpn] = Pte { valid: true, ppn };
fn map(pt: &mut [Pte], vpn: usize, ppn: usize) {
    // ← 占位：什么都没映射，translate 将返回 None → ALIAS_FAIL
}
...
fn sub_alias() -> bool {
    ...
    // TODO(2): 让 vpn_a、vpn_b 映射到同一物理页 ppn_shared（别名 = 两 PTE 同 PPN）。
    // HINT: 两行 map：map(&mut pt, vpn_a, ppn_shared); map(&mut pt, vpn_b, ppn_shared);

    let va_a = vpn_a * PAGE_WORDS + 1;
...
fn do_mmap(w: &mut World, pt: &mut [Pte], vpn: usize, key: u32, flags: u32) {
    // TODO[a] MAP_SHARED：查 w.reg 命名段注册表，命中复用同一 ppn；未命中再 frame_alloc 并登记。
    //   if let Some(&(_, p)) = w.reg.iter().find(|(k,_)| *k == key) { p }
    //   else { let p = w.frame_alloc(); w.reg.push((key, p)); p }
    // ELSE[b] MAP_PRIVATE：永远 w.frame_alloc() 一块新（零）帧。
    //
    // 求出 ppn 后：map(pt, vpn, ppn);
    // ← 占位：没建立任何映射 → translate None → SHARED_FAIL
}

fn sub_mmap() -> bool {
    let mut w = World::new();
    let mut pt1 = vec![Pte::default(); 16];
    let mut pt2 = vec![Pte::default(); 16];
    let key = 0x55u32;
...
/// TODO(4): 生产者一步——先写满 data，**最后**才置 ready。
/// HINT: for i in 0..MB_N { mb.data[i] = payload[i]; }  然后  mb.ready = true;
fn producer_step(mb: &mut Mailbox, payload: &[u32; MB_N]) {
    // ← 占位：没写 data、没置 ready → 消费者永远拿不到 → MAILBOX_FAIL
}

/// TODO(5): 消费者一步——仅当 ready 才拷贝；拷贝后清 ready。
/// HINT: if !mb.ready { return None; } let got = mb.data; mb.ready = false; Some(got)
...
/// TODO(6a): 入队（生产侧）。满则拒绝返回 false；否则写 tail、抬 tail、count+1。
/// HINT: if r.count==CAP { return false; } r.buf[r.tail]=x; r.tail=(r.tail+1)%CAP; r.count+=1; true
fn ring_push(r: &mut Ring, x: u32) -> bool {
    false // ← 占位：永远入队失败 → RING_FAIL
}

/// TODO(6b): 出队（消费侧）。空则 None；否则读 head、抬 head、count-1。
/// HINT: if r.count==0 { return None; } let x=r.buf[r.head]; r.head=(r.head+1)%CAP; r.count-=1; Some(x)
...
fn sub_mmio() -> bool {
    ...
    let stream = [0xD0u32, 0xD1, 0xD2, 0xD3, 0xD4];
