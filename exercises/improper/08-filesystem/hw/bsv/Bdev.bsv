// RAM 块设备模型 —— 硬件路径（BlueSpec SV）。
// 每个地址 = 一个「块」，data = 块内容（简化为 1 字/块）。
// 软件 BlockDev::write_block/read_block 的硬件同构：wr 写、rd 读。你只需填 wr/rd。
package Bdev;
...
module mkBdev (Bdev_ifc);
   ...
   method Action wr(Bit#(6) addr, Bit#(32) data);
      // TODO: 把 data 落到第 addr 个块：mem[addr] <= data;
      mem[addr] <= data & 0;  // ← 占位：写入 0 → 读回不符 → BDEV_FAIL
   endmethod

   method Bit#(32) rd(Bit#(6) addr);
      // TODO: 读出第 addr 个块：return mem[addr];
      return mem[addr];
   endmethod
endmodule
