// 16e · 中断聚合 + 多核仲裁 - 设备侧（Verilog，学生填空版）。
...
// 你只需填两处：claim 授予条件 claim_ok（①）与 IPI 的 MSIP 置位（②）；读多路器/时序骨架勿改。
...
module intc_agg (
    ...
    // ── 学生填①：仲裁授予 - 仅当“本次有 claim 请求”且“有 pending”且“gateway 空闲(~inflight_r)”时授予 ──
    //   HINT: wire claim_ok = claim_req & pending_r & ~inflight_r;
    wire claim_ok = 1'b0; // ← 占位：永不授予 → 没有 winner → 仲裁失败
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            //   HINT: if (ipi_send) msip_r[ipi_target] <= 1'b1;
            if (ipi_send)  msip_r[ipi_target]     <= 1'b0; // ← 占位：没置位 → 从核见不到 IPI
