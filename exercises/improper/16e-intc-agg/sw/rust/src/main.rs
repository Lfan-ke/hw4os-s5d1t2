//! 16e · 中断聚合 + 多核仲裁 - Rust（学生填空版，类型化原子）。
...
//! 你只需填两处：claim 的 AtomicU32::swap（①）与 IPI 的 MSIP store（②）；其余 harness 勿改。
...
impl Plic {
    ...
    fn claim(&self, hart: usize) -> u32 {
        ...
        // TODO①: 用 self.inflight.swap(1, Ordering::AcqRel) 原子换值；旧值为 0 才是赢家。
        //   HINT: if self.inflight.swap(1, Ordering::AcqRel) == 0 { IRQ_ID } else { 0 }
        IRQ_ID // ← 占位：恒返回 IRQ_ID（没真正抢 gateway）→ 每个 hart 都以为自己赢 → 仲裁崩溃
    }
...
fn send_ipi(msip: &[AtomicU32], target: usize) {
    // TODO②: 用 msip[target].store(1, Ordering::Relaxed) 置目标 MSIP（点亮软件中断）。
    //   HINT: msip[target].store(1, Ordering::Relaxed);
    let _ = (msip, target); // ← 占位：没敲 IPI → 从核永远见不到软件中断
}
