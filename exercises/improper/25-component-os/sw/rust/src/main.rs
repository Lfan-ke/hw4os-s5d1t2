...
fn build_kernel(cfg: &KernelConfig) -> Kernel {
    ...
    //   has_sched = (cfg.sched_kind != SchedKind::NoSched)，sched = make_scheduler(cfg.sched_kind)
    ...
    Kernel {
        ...
        console_len: 0,
        alloc: make_allocator(AllocKind::Bump),
        astate: AllocState::new(),
        sched: make_scheduler(SchedKind::Fifo),
        has_sched: false,
        syscall_boundary: false,
        traps: 0,
    }
}
...
fn freelist_alloc(s: &mut AllocState, n: usize) -> i64 {
    ...
    //       否则 off = (s.slots as usize * SLOT) as i64；s.slots += 1；返回 off。
    let _ = (s, n);
    -1 // 占位：永远 OOM（换上它的 SWAP 会失败）
}
