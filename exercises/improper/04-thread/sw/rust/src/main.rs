...
//! 你只需填 4 个函数体；下方测试 harness（构造 + 不变量校验 + PASS 打印）勿改。
...
fn ctx_save(vcpu: &Vcpu, cur: &mut Context) {
    // TODO[a]：具名字段逐个搬——cur.gpr = vcpu.regs.gpr; cur.sepc = vcpu.regs.sepc; cur.sstatus = ...; cur.sp = ...
    // ELSE[b]：整体赋值最直白——*cur = vcpu.regs;
    // HINT：“保存”= 把当前 vCPU 的整套寄存器现场整存进 cur。
    // 占位：什么也不存 → 判 FAIL
}

/// 把 next 的整套寄存器整载回 vCPU（与 save 相反）。
fn ctx_restore(vcpu: &mut Vcpu, next: &Context) {
    // TODO: vcpu.regs = *next;（把 next 这套上下文整组载回当前 vCPU）
    // 占位：什么也不载 → 判 FAIL
}

/// 协作式切换：先存当前现场到 cur，再把 next 载入当前。
fn ctx_switch(vcpu: &mut Vcpu, cur: &mut Context, next: &Context) {
    // TODO: ctx_switch = 先 ctx_save(当前→cur)，再 ctx_restore(next→当前)。
    // 占位：什么也不切 → 判 FAIL
}
...
fn spawn_shared_pair(p: Rc<RefCell<Process>>, other: Rc<RefCell<Process>>) -> (Tcb, Tcb) {
    ...
    // ELSE[b]：用 Arc<Mutex<_>>（多线程版同理：把 Rc→Arc、RefCell→Mutex）。
    // 占位：t2 误指向另一个进程 other → 两线程不共享 → 判 FAIL
    let t1 = Tcb { tid: 1, ctx: Context::zero(), proc: Rc::clone(&p) };
    let t2 = Tcb { tid: 2, ctx: Context::zero(), proc: Rc::clone(&other) };
    (t1, t2)
}
