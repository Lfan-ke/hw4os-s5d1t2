...
static void map1(Pt1 *pt, uint64_t vpn, uint64_t ppn) {
    ...
    /* TODO: pt->vpn[pt->n]=vpn; pt->ppn[pt->n]=ppn; pt->n++; */
}
/* 翻译：va = vpn(高位)|off(低 12 位)；命中返回 1 且 *pa=ppn<<12|off，否则返回 0。 */
static int translate(const Pt1 *pt, uint64_t va, uint64_t *pa) {
    (void)pt; (void)va; (void)pa;
    /* TODO: vpn=va>>12、off=va&0xfff；遍历 pt 找 vpn；命中写 *pa 并 return 1。
     * HINT: for (i...) if (pt->vpn[i]==vpn) { *pa=(pt->ppn[i]<<12)|off; return 1; } */
    return 0; /* ← 占位（永远 miss） */
}
...
static uint64_t route(World2 *w, Pt1 *pt, uint64_t va) {
    (void)w; (void)pt; (void)va; (void)&w2_alloc;
    /* TODO[a]: if (in_direct(va)) return va;   (identity 映射)
     * ELSE[b]: 否则 translate(pt,va,&pa)；miss 时 w2_alloc 一帧、map1 后再 translate。
     * HINT:
     *   if (in_direct(va)) return va;
     *   uint64_t pa; if (translate(pt, va, &pa)) return pa;
     *   uint64_t ppn; if (w2_alloc(w, &ppn)) { map1(pt, va>>12, ppn); if (translate(pt, va, &pa)) return pa; }
     *   return ~0ull; */
    return 0; /* ← 占位 */
}
...
static void map2(World4 *w, uint64_t va, uint64_t pa) {
    (void)w; (void)va; (void)pa;
    /* TODO: l1=(va>>22)&0x3ff、l2=(va>>12)&0x3ff；若 w->pd[l1]<0 建一张新表（清零、记下标、ntab++）；
     *       w->tab[t][l2] = (pa>>12)+1。 */
}
/* 两级 walk：pde=PD[l1] → pt=tab[pde] → pte=pt[l2] → pa=ppn<<12|off。命中返回 1。 */
static int walk2(const World4 *w, uint64_t va, uint64_t *pa) {
    (void)w; (void)va; (void)pa;
    /* TODO[a]: 命中（pd[l1]>=0 且 pte!=0）→ *pa=((pte-1)<<12)|off; return 1。
     * ELSE[b]: 任一级缺失 → return 0（fault）。 */
    return 0; /* ← 占位 */
}
...
static int sv39_walk(const Sv39 *s, uint64_t va, uint64_t *pa, uint64_t *flags) {
    (void)s; (void)va; (void)pa; (void)flags;
    /* TODO: off=va&0xfff；vpn[0..3]=(va>>12/21/30)&0x1ff；t=0（根）。
     *   for (level=2; level>=0; level--) { pte=s->tabs[t][vpn[level]];
     *     if ((pte&PTE_V)==0) return 0;
     *     if (pte & (PTE_R|PTE_W|PTE_X)) { *pa=((pte>>10)<<12)|off; *flags=pte&0x1f; return 1; }
     *     t = (int)(pte>>10); } */
    return 0; /* ← 占位 */
}
