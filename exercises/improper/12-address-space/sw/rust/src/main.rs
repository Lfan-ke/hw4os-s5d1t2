...
fn map1(pt: &mut Pt1, vpn: u64, ppn: u64) {
    // TODO: 把 (vpn, ppn) 追加进 pt.ents。
    // HINT: pt.ents.push((vpn, ppn));
}
/// 翻译：va = vpn(高位)|off(低 12 位)；查到映射则 pa=ppn<<12|off，否则 None。
fn translate(pt: &Pt1, va: u64) -> Option<u64> {
    // TODO: 拆出 vpn=va>>12、off=va&0xfff；在 pt.ents 里找 vpn；命中返回 Some(ppn<<12|off)。
    // HINT: for &(v, p) in &pt.ents { if v == vpn { return Some((p << 12) | off); } }
    None // ← 占位（永远 miss）
}
...
fn route(w: &mut World2, pt: &mut Pt1, va: u64) -> u64 {
    // TODO[a]: 若 in_direct(va)，直接 return va;（identity 映射）
    // ELSE[b]: 否则查 translate(pt, va)；miss 时 w.alloc() 一帧、map1 后再 translate。
    // HINT:
    //   if in_direct(va) { return va; }
    //   match translate(pt, va) {
    //       Some(pa) => pa,
    //       None => { let vpn = va >> 12; if let Some(ppn) = w.alloc() { map1(pt, vpn, ppn); translate(pt, va).unwrap_or(u64::MAX) } else { u64::MAX } }
    //   }
    0 // ← 占位
}
...
fn slot_addr(hart: usize) -> u64 {
    // TODO: 返回 SMP_BASE + hart*SLOT。
    // HINT: SMP_BASE + (hart as u64) * SLOT
    0 // ← 占位
}
/// 屏障后由 0 号核归约：把各核私有槽位求和（无锁，因互不写同一槽）。
fn reduce_slots(slots: &[u64]) -> u64 {
    // TODO: 返回 slots 各项之和。
    // HINT: slots.iter().copied().sum()
    0 // ← 占位
}
...
fn map2(w: &mut World4, va: u64, pa: u64) {
    // TODO: l1=(va>>22)&0x3ff、l2=(va>>12)&0x3ff；若 w.pd[l1]<0 就新建一张二级表并记下标；
    //       然后 w.tables[t][l2] = (pa>>12)+1。
    // HINT:
    //   let l1 = ((va >> 22) & 0x3ff) as usize; let l2 = ((va >> 12) & 0x3ff) as usize;
    //   if w.pd[l1] < 0 { w.pd[l1] = w.tables.len() as i64; w.tables.push(vec![0u64; 1024]); }
    //   let t = w.pd[l1] as usize; w.tables[t][l2] = (pa >> 12) + 1;
}
/// 两级 walk：pde=PD[l1] → pt=tables[pde] → pte=pt[l2] → pa=ppn<<12|off。
fn walk2(w: &World4, va: u64) -> Option<u64> {
    // TODO[a]: 命中（pd[l1]>=0 且 pte!=0）则 Some(((pte-1)<<12)|off)。
    // ELSE[b]: 任一级缺失返回 None（fault）。
    // HINT: 先取 l1/l2/off；if w.pd[l1] < 0 { return None; } …
    None // ← 占位
}
...
fn sv39_walk(s: &Sv39, va: u64) -> Option<(u64, u64)> {
    // TODO: off=va&0xfff；vpn[0..3] = (va>>12/21/30)&0x1ff；t=0（根）。
    //       for level in (0..3).rev() { 取 pte=s.tabs[t][vpn[level]]；
    //         若 V==0 → None；若带 R/W/X（叶）→ Some(((pte>>10)<<12)|off, pte&0x1f)；否则 t=pte>>10 下探。 }
    // HINT: 见参考解；rwx = pte & (PTE_R|PTE_W|PTE_X)。
    None // ← 占位
}
