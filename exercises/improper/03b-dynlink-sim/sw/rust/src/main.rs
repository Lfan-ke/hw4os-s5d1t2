...
fn check_bind() -> bool {
    ...
    let now_cnt = now_bind(&mut p_now, &libc);
    let lazy_cnt = lazy_bind(&mut p_lazy, &libc, &["puts"]);
    let mut ok = true;
    if now_cnt != 3 {
