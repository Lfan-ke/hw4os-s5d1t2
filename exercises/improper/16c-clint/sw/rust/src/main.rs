...
//! 你只需在 poll_mip 里把 CLINT 状态翻译成 mip 挂起位（MTIP/MSIP）；其余 harness 勿改。
#![allow(dead_code)]
...
fn poll_mip(c: &Clint) -> Mip {
    // ── TODO: 把 CLINT 状态翻译成 mip 挂起位 ──
    //   timer:    if c.mtime.get() >= c.mtimecmp.get() { mip |= Mip::MTIP; }
    //   software: if c.msip.get() & 1 != 0            { mip |= Mip::MSIP; }
    let _ = c;
    Mip::empty() // ← 占位：未翻译任何挂起位 → 无 TIMER_PASS/SOFT_PASS
}
