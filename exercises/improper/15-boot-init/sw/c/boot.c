...
static void boot_init(Device *d) {
     ...
     *   ② mmio_write(d, MMIO_CLKDIV, <合法值 1..15>);    配时钟，0 会 BADCLK
     *   ③ mmio_write(d, MMIO_CTRL, CTRL_EN | CTRL_LE);  使能
     *   ④ while (!(mmio_read(d, MMIO_STATUS) & ST_READY)) { }   忙等轮询 READY
...
static void boot_ctor(void) {
    /* TODO: 让 boot_init 先于 app_main 跑——在此调用 boot_init(&g_dev)。
     * 占位：留空 → g_dev 仍锁着 → app_main 抢跑 → 打 BOOT_FAULT。 */
}
