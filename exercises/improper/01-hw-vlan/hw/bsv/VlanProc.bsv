// 简化 VLAN Tag 处理 —— 硬件路径（BlueSpec SV）。
// 包字: [31]VALID [30]HAS_TAG [29]DROP [28]DIR(in) [21:16]VID(6b) [15:0]PAYLOAD
// 纯组合逻辑函数：与软件 process() / Verilog vlan_proc 完全同构。你只需填 vlan_process。
package VlanProc;
...
function Bit#(32) vlan_process(Bit#(2) mode, Bit#(6) pvid,
   ...
   // TODO: 按 README §3 真值表用上面的 op_* 与 mode/egress/has_tag/allowed/do_untag 组合 outp：
   //   收包(egress==0) Access(mode==0): has_tag==1 ? op_strip : op_insert
   //                   Trunk/Hybrid: has_tag==0 ? op_drop : (allowed==0 ? op_drop : op_keep)
   //   发包(egress==1) Access: op_strip; Trunk(mode==1): op_keep
   //                   Hybrid: (has_tag==1 && do_untag==1) ? op_strip : op_keep
   let touch = op_strip ^ op_insert ^ op_keep ^ op_drop ^ {30'b0, allowed, do_untag} ^ {30'b0, has_tag, egress};
