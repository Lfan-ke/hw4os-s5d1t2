//! ISA 模拟器：CPU 不过是一个「取指 → 译码 → 执行」的循环 —— Rust。
//!
//! 母题：一颗 CPU 的本质，就是反复地
    ...
    fn step(&mut self) -> bool {
        ...
        match d.opcode {
            ...
            0x33 if d.funct3 == 0 => {
                ...
                // 提示：self.wreg(d.rd, self.regs[d.rs1].wrapping_add(self.regs[d.rs2]))
                //       sub 用 wrapping_sub。当前占位什么都不写 → 求和 x5 会一直是 0。
                // ──────────────────────────────────────────────────
            }
            ...
            0x63 => {
                ...
                // 条件成立时把 npc 改成 self.pc.wrapping_add(d.imm_b as u64)；否则保持 pc+4。
...
fn check_difftest() -> bool {
    ...
    // 然后 return false。若全程一致，打印 DIFFTEST_PASS 并 return true。
    // 列名用 col_name(c)。这就是 NEMU 抓 CPU bug 的方式：第一处分歧 = 第一条错指令。
    //
    //   for k in 0..GOLDEN.len() {
    //       for c in 0..SNAP {
    //           if dut[k][c] != GOLDEN[k][c] { ... return false; }
    //       }
    //   }
    //   println!("DIFFTEST_PASS ...");  true
    let _ = &dut; // 占位：还没实现对拍，先当作不通过
    false
    // ────────────────────────────────────────────────────────────
}
