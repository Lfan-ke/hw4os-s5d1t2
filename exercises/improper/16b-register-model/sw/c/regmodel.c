 ...
 * 你只需填两处汇编骨架：raw_lw/raw_sw（①）与 readl/writel 宏（④）；其余 harness 勿改。
 */
#include <stdio.h>
#include <stdint.h>
...
static inline uint32_t raw_lw(uintptr_t a) {
    /* TODO①: 用 `fence rw,rw` + `lw %0,0(%1)` 从 a 处读一个字并返回。
     *   HINT: __asm__ volatile("fence rw,rw\n\t lw %0,0(%1)" : "=r"(v) : "r"(a) : "memory"); */
    (void)a;
    return 0; /* ← 占位：读不到 magic → RAW_FAIL */
}
static inline void raw_sw(uintptr_t a, uint32_t v) {
    /* TODO①: 用 `sw %0,0(%1)` + `fence rw,rw` 把 v 写到 a 处。
     *   HINT: __asm__ volatile("sw %0,0(%1)\n\t fence rw,rw" : : "r"(v), "r"(a) : "memory"); */
    (void)a; (void)v; /* ← 占位：写不进去 */
}
...
/* TODO④: 用内联汇编 lw/sw 实现下面两个宏（封成 Linux 风格的 readl/writel）。
 *   HINT readl(a):  ({ uint32_t __v; __asm__ volatile("lw %0,0(%1)" : "=r"(__v) : "r"((uintptr_t)(a)) : "memory"); __v; })
 *   HINT writel(v,a): __asm__ volatile("sw %0,0(%1)" : : "r"((uint32_t)(v)), "r"((uintptr_t)(a)) : "memory"); */
#define readl(a)     ((void)(uintptr_t)(a), 0u)                                    /* ← 占位 */
#define writel(v, a) do { (void)(uint32_t)(v); (void)(uintptr_t)(a); } while (0)   /* ← 占位 */
