// 16b · 寄存器模型 - 设备侧（BlueSpec SV，学生填空版）。
...
// 你只需在 read 方法里把“字段位选”从 ctrl 选出来（下方 ── 学生填① ──）；读多路器/写方法/tb 勿改。
package RegDev;
...
module mkRegDev (RegDev);
   ...
   method Word read(Addr addr);
      // ── 学生填①：字段位选 - 按 §2.1 从 ctrl 选出各字段 ──
      Bit#(1) en = 0; // TODO: ctrl[0]
      Bit#(1) ie = 0; // TODO: ctrl[1]
      // 以下由字段派生（勿改）：
