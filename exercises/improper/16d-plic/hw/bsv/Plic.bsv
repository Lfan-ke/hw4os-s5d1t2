// 16d · 核外中断 PLIC - 设备侧（BlueSpec SV，学生填空版）。
...
// 你只需补全 arbitrate（仲裁器核心）；mkPlic 的方法/读写、mkTbPlic 测试 harness 勿改。
package Plic;
...
function Tuple2#(Bit#(3), Bit#(2))
   ...
   // TODO: 算每源 eligibility，再取最高 prio（同 prio 取最小 id）。
   //   HINT:
   //     Bit#(3) bid = 0; Bit#(2) bp = 0;
   //     Bool e1 = (pend[1]==1) && (ena[1]==1) && (p1 > thr); ...（e2/e3/e4 同理）
   //     if (e4 && (p4 >= bp)) begin bp = p4; bid = 4; end  ...（e3/e2/e1 同理，自高 id 向低）
   //     return tuple2(bid, bp);
   return tuple2(0, 0); // ← 占位：永远无源可路由 → ROUTE_FAIL
endfunction
