...
impl Plic {
    ...
    // 组合仲裁器：取最高优先级的合格源，同优先级取最小 id。
    fn arbitrate(&self) -> u32 {
        // TODO: 合格源 = pending & enable & (prio > threshold)；
        //   在所有合格源里取最高优先级（同优先级取最小 id，自底向上用 strict `>`）。
        //   HINT:
        //     let mut best = 0u32; let mut bp = 0u32;
        //     for id in 1..=4u32 {
        //         let bit = Sources::from_bits_truncate(1u32 << id);
        //         if !self.pending.contains(bit) { continue; }
        //         if !self.enable.contains(bit) { continue; }
        //         let p = self.prio[id as usize];
        //         if p <= self.threshold { continue; }
        //         if p > bp { bp = p; best = id; }
        //     }
        //     best
        0 // ← 占位：永远无源可路由 → ROUTE_FAIL
    }
