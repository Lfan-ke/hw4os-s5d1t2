 ...
 * 你只需填四处 TODO + board_nodes 的 compatible/reg + 第三个 REGISTER_DRIVER；harness 勿改。
 */
#include <stdio.h>
#include <stdint.h>
#include <string.h>
...
/* 模拟 readl/writel：对“地址”的 volatile 读写（填好 TODO 后即被你的驱动调用）。 */
static uint32_t mmio_read(const MmioDevice *d, unsigned off) __attribute__((unused));
static uint32_t mmio_read(const MmioDevice *d, unsigned off) { return bus_read(d, off); }
static void mmio_write(MmioDevice *d, unsigned off, uint32_t v) __attribute__((unused));
...
static int driver_probe(const MmioDevice *d) {
    /* TODO: return mmio_read(d, REG_ID) == DEV_MAGIC; */
    (void)d;
    return 0; /* ← 占位：未探到 → PROBE_FAIL */
}
...
static void driver_io(MmioDevice *d, const uint8_t *msg, int n) {
     ...
     *   mmio_write(d, REG_CTRL, CTRL_ENABLE);                       // 置使能
     *   while ((mmio_read(d, REG_STATUS) & STATUS_READY) == 0) { ... } // 轮询(设上限防死等)
     *   for (int i = 0; i < n; i++) mmio_write(d, REG_DATA, msg[i]); // 逐字节突发写
     * ELSE[b] 也可：读“可写计数”后一次性突发写——对外输出一致。 */
    (void)d; (void)msg; (void)n;
...
static int board_nodes(DevNode *out) {
    ...
    strcpy(out[0].compatible, "TODO,fillme");
    out[0].base = 0;
    out[0].size = 0;
    /* TODO: gpio@10002000：compatible 应为 "acme,gpio"，reg=<0x10002000 0x1000> */
    strcpy(out[1].name, "gpio@10002000");
    strcpy(out[1].compatible, "TODO,fillme");
    out[1].base = 0;
    out[1].size = 0;
    /* lamp@10003000（给定，勿改）：16.2 时未知，16.3 注册 blink-v2 后被发现。 */
...
static int match_and_probe(const DevNode *nodes, int nn, const Driver *drv, int nd,
    ...
    int skipped = 0;
    *mcount = 0;
    /* TODO: 遍历 nodes，对每个节点在 drv[] 里逐条 strcmp(compatible) 精确匹配：
     *   命中 → drv[j].probe(base,size); printf("PROBE %s@%x\n", name, base);
     *          snprintf(matched[*mcount],32,"%s@%x",name,base); (*mcount)++; hit=1; break;
     *   一个都不命中 → skipped++（未知 compatible 走 fallback 跳过）
     * 二选一：// TODO[a] libfdt 解析 / // ELSE[b] 手写遍历（FDT 已给好）。 */
    (void)nodes; (void)nn; (void)drv; (void)nd; (void)matched;
...
/* ── 学生填 16.3：在此新增第三个驱动的注册标注（框架遍历代码一行不改）──
 * TODO: 取消下一行注释，让 acme,blink-v2 被链接段自发现并 probe：
...
/* DevFile 的 FileLike read/write（复用“设备即文件”抽象；填好 user_read/write 后即被调用）。 */
static uint32_t file_read(const DevFile *f) __attribute__((unused));
static uint32_t file_read(const DevFile *f) { return f->state; }
static void file_write(DevFile *f, uint32_t v) __attribute__((unused));
...
static void bind_all(Bus *bus, const DevNode *nodes, int nn, const Driver *drv, int nd) {
     ...
     *   int idx = <同名驱动已绑定数>;                                  // 实例编号
     *   snprintf(bus->devs[bus->n].path,24,"/dev/%s%d",drv[j].name,idx);
     *   bus->devs[bus->n].file = inst; bus->n++; break; */
    (void)nodes; (void)nn; (void)drv; (void)nd;
    /* 占位：什么都不绑定 → BIND_FAIL */
}

/* 用户态 open+read：找到 /dev 路径的 FileLike，转发 read（不直接碰 MMIO）。 */
static int user_read(const Bus *bus, const char *path, uint32_t *out) {
    /* TODO: 在 bus->devs 里按 path 找到设备，*out = file_read(&...->file); return 1; */
    (void)bus; (void)path; (void)out;
    return 0; /* ← 占位 */
}
static int user_write(Bus *bus, const char *path, uint32_t v) {
    /* TODO: 在 bus->devs 里按 path 找到设备，file_write(&...->file, v); return 1; */
    (void)bus; (void)path; (void)v;
    return 0; /* ← 占位 */
}
