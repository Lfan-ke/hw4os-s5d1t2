...
//!
//! 你只需填 raise / unmask 两个函数体（标 TODO 处）；下方 run_handler 与测试 harness 勿改。
...
fn raise_sig(p: &mut Proc, sig: usize) {
    // TODO: 补全「投递 vs 入 pending」的判断。
    //   被屏蔽(is_masked(p,sig)) → 入 pending：p.pending |= bit(sig);
    //   否则                      → 立刻投递：run_handler(p, sig);
    // 下面这版无脑投递（忘了判屏蔽），会让 MASK/PENDING 失败：
    run_handler(p, sig); // ← 占位：请改成「先判 is_masked 再决定」
}

/// unmask(sig)：解除屏蔽，并把屏蔽期间攒下的 pending「补投递」出去。
fn unmask_sig(p: &mut Proc, sig: usize) {
    // TODO: 1) 清掉 mask 位：p.mask &= !bit(sig);
    //       2) 若还挂着同号 pending(is_pending) → 清 pending 位、run_handler 补投递一次。
    p.mask &= !bit(sig); // ← 占位：只放开了屏蔽，还差「补投递」那一步
}
