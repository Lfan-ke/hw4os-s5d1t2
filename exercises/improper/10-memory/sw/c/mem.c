...
static uint64_t blk_read(Dev *dev, int blk) {
    (void)dev; (void)blk;
    /* TODO: dev->cost += dev->per_access; return dev->data[blk]; */
    return 0; /* ← 占位：直接返回 0，DEV_PROBE 会判 FAIL */
}

/* 写一块：累加访问代价，写入该块。 */
static void blk_write(Dev *dev, int blk, uint64_t val) {
    (void)dev; (void)blk; (void)val;
    /* TODO: dev->cost += dev->per_access; dev->data[blk] = val; */
...
static void stage_in(Dev *fast, Dev *slow, int n) {
    (void)fast; (void)slow; (void)n;
    /* TODO: for i in [0,n): blk_write(fast, i, blk_read(slow, i)); */
...
static void stage_out(Dev *fast, Dev *slow, int n) {
    (void)fast; (void)slow; (void)n;
    /* TODO: for i in [0,n): blk_write(slow, i, blk_read(fast, i)); */
...
static int translate(Pager *p, int vpn) {
    (void)p; (void)vpn;
    /* TODO: 实现缺页路径——
     *   1) 命中（p->ptes[vpn].present）：return p->ptes[vpn].frame。
     *   2) 缺页且有空闲帧（p->free_top < p->num_frames）：frame = p->free_top++。
     *   3) 缺页且帧满：选 victim 帧并换出——
     *        // TODO[a] FIFO：frame = p->fifo[p->fifo_head]; 推进 fifo_head、fifo_len--。
     *        // ELSE[b] Clock：用 ref 位 + 指针的二次机会算法选 victim。
     *      vvpn = p->owner[frame]；若 p->ptes[vvpn].dirty，
     *      blk_write(&p->slow, vvpn, blk_read(&p->fast, frame)) 回写 swap；
     *      再 p->ptes[vvpn].present = 0; p->owner[frame] = -1。
     *   4) 换入目标页：blk_write(&p->fast, frame, blk_read(&p->slow, vpn));
     *      置 present/frame、清 dirty；p->owner[frame]=vpn；把 frame 压入 fifo。返回 frame。
     * HINT: swap 槽号直接等于 vpn。 */
    return 0; /* ← 占位：永远返回帧 0，PAGEIN/SWAPOUT 会判 FAIL */
}

/* 同步：退出前把所有"驻留且脏"的页刷回 SLOW，保证持久。 */
static void sync_all(Pager *p) {
    (void)p;
    /* TODO: 遍历所有 vpn，若 present 且 dirty，
     *       blk_write(&p->slow, vpn, blk_read(&p->fast, frame)) 回写并清 dirty。 */
...
static void addr_route(int la, int fast_size, int *dev, int *off) {
    (void)la; (void)fast_size;
    /* TODO: la < fast_size → *dev=0, *off=la；否则 *dev=1, *off=la-fast_size。 */
    *dev = 0;
    *off = 0; /* ← 占位：永远落 FAST off0，UNIFIED 会判 FAIL */
}
