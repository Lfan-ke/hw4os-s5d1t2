//! 内存管理：分层、Swap 与统一地址空间 —— Rust（学生填空版）。
//!
//! 两块"设备"= 两个软件数组：
...
//! 占位实现能编译、能运行，但会判 FAIL——把每个 TODO 换成正确逻辑即可。
...
fn blk_read(dev: &mut Dev, blk: usize) -> u64 {
    // TODO: 先累加访问代价（dev.cost += dev.per_access），再返回 dev.data[blk]。
    // HINT: 代价计数器是"快/慢分层"被打印验证的关键，别忘了加。
    0 // ← 占位：直接返回 0，DEV_PROBE 会判 FAIL
}

/// 写一块：累加访问代价，写入该块。
fn blk_write(dev: &mut Dev, blk: usize, val: u64) {
    // TODO: 先累加访问代价（dev.cost += dev.per_access），再写 dev.data[blk] = val。
    // ← 占位：什么都不做，DEV_PROBE 会判 FAIL
}
...
fn stage_in(fast: &mut Dev, slow: &mut Dev, n: usize) {
    // TODO: for i in 0..n { 从 slow 第 i 块 blk_read，再 blk_write 到 fast 第 i 块 }
    // ← 占位：什么都不搬，SCENARIO_A 会判 FAIL
}

/// 搬出：把 FAST 上 [0,n) 的结果写回 SLOW 的 [0,n)（持久化）。
fn stage_out(fast: &mut Dev, slow: &mut Dev, n: usize) {
    // TODO: for i in 0..n { 从 fast 第 i 块 blk_read，再 blk_write 到 slow 第 i 块 }
    // ← 占位：什么都不搬，结果不持久 → SCENARIO_A 会判 FAIL
}
...
impl Pager {
    ...
    fn translate(&mut self, vpn: usize) -> usize {
        // TODO: 实现缺页路径——
        //   1) 命中（self.ptes[vpn].present）：直接 return self.ptes[vpn].frame。
        //   2) 缺页且有空闲帧（self.free.pop()）：用该空闲帧。
        //   3) 缺页且帧满：选 victim 帧并换出——
        //        // TODO[a] FIFO：let victim = self.fifo.pop_front().unwrap();
        //        // ELSE[b] Clock：用 ref 位 + 指针的二次机会算法选 victim。
        //      取 vvpn = self.owner[victim]；若 self.ptes[vvpn].dirty，
        //      把帧值 blk_read(fast, victim) 后 blk_write(slow, vvpn, ..) 回写 swap；
        //      再 self.ptes[vvpn].present = false; self.owner[victim] = None。
        //   4) 换入目标页：val = blk_read(slow, vpn); blk_write(fast, frame, val);
        //      置 self.ptes[vpn] = Pte{present:true, frame, dirty:false}；
        //      self.owner[frame]=Some(vpn); self.fifo.push_back(frame)。返回 frame。
        // HINT: swap 槽号直接等于 vpn。
        0 // ← 占位：永远返回帧 0，PAGEIN/SWAPOUT 会判 FAIL
    }

    /// 同步：退出前把所有"驻留且脏"的页刷回 SLOW，保证持久。
    fn sync_all(&mut self) {
        // TODO: 遍历所有 vpn，若 present 且 dirty，把帧值 blk_read(fast, frame)
        //       blk_write(slow, vpn, ..) 回写慢设备，并清 dirty。
        // ← 占位：什么都不刷，SYNC 会判 FAIL
    }
}
...
fn addr_route(la: usize, fast_size: usize) -> (u32, usize) {
    // TODO: la < fast_size → 落 FAST，返回 (0, la)；
    //       否则落 SLOW，返回 (1, la - fast_size)。
    (0, 0) // ← 占位：永远落 FAST off0，UNIFIED 会判 FAIL
}
