// 平坦大内存地址译码器 —— 硬件路径（BlueSpec SV 学生填空版）。
...
// 占位实现 0 warning 可编译，但 DECODE 判 FAIL。
package MemDecode;

// ── 核心逻辑：学生只需填这个函数体 ────────────────────────────────
function Bit#(10) decode(Bit#(8) la, Bit#(8) fast_size);
   // TODO: 用 la 与 fast_size 比较，给三段赋值（与软件 addr_route 等价）：
   //   la < fast_size : cs_fast=1, cs_slow=0, off = la
   //   否则           : cs_fast=0, cs_slow=1, off = la - fast_size
   // 最后 return { cs_fast, cs_slow, off };
   Bit#(1) cs_fast = 1'b0;
   Bit#(1) cs_slow = 1'b0;
   Bit#(8) off     = la & fast_size & 8'b0; // ← 占位：触碰 la/fast_size 后恒 0，DECODE 判 FAIL
