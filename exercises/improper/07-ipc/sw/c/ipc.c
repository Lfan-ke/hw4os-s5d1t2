 ...
 *
 * 你只需填 8 个函数体（标 TODO 处）；下方测试 harness 勿改。
 */
#include <stdio.h>
#include <stdint.h>
#include <pthread.h>
...
static uint32_t b_finish(uint32_t result) {
    /* TODO: 置 DONE，并把 result 低 16 位放进 [15:0]。 */
    /* HINT: return DONE | (result & RESULT_MASK); */
    (void)result;
    return 0; /* ← 占位 */
}
...
static int a_poll(uint32_t ctrl, uint32_t *result) {
    /* TODO: *result = ctrl 的低 16 位；返回 (ctrl & DONE) != 0。 */
    /*   TODO[a] 一次性解包 / ELSE[b] 先判 DONE 再取 RESULT */
    (void)ctrl;
    *result = 0;
    return 0; /* ← 占位 */
}
...
static int try_lock(uint32_t *lock) {
    /* TODO: 调 tas(*lock,&newv) 得 got，把 newv 写回 *lock，返回 got。 */
    /*   TODO[a] 基于 tas 自旋 / ELSE[b] 基于 cas(cur,exp,new) */
    (void)lock;
    return 0; /* ← 占位 */
}
...
static int down_sem(int count, int *out) {
    /* TODO[a] 阻塞式：int c = count - 1; *out = c; return c >= 0; */
    /* ELSE[b] 自旋式：count>0 才减一并 return 1，否则 *out=count 返回 0。 */
    (void)count;
    *out = 0;
    return 0; /* ← 占位 */
}
...
static uint32_t b_step(uint32_t ctrl, uint32_t job) {
    /* TODO: 若 ctrl 的 START 位为 1，返回 DONE | (job & RESULT_MASK)（同时清 START）；
     *       否则原样返回 ctrl。 */
    (void)job;
    return ctrl; /* ← 占位 */
}
...
static uint32_t a_step(uint32_t ctrl, uint32_t phase, uint32_t *phase_out, uint32_t *post_out) {
    /* TODO:
     *   phase==0: *phase_out=1; *post_out=0; return ctrl | START;
     *   phase==1 且 DONE=1: *post_out=result*2; *phase_out=0; return ctrl & ~DONE;
     *   phase==1 且 DONE=0: *phase_out=1; *post_out=0; return ctrl;
     *   TODO[a] 先用后清 DONE / ELSE[b] 先清后用：同一断言下等价。 */
    *phase_out = phase;
    *post_out = 0;
    return ctrl; /* ← 占位 */
}
