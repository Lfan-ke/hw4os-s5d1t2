//! 进程通信：原子操作、锁与「A 等 B 置位」的完成握手 —— Rust。
//!
...
fn b_finish(result: u32) -> u32 {
    // TODO: 置 DONE 位，并把 result 的低 16 位放进 [15:0]（BUSY 不置即为清）。
    // HINT: DONE | (result & RESULT_MASK)
    0 // ← 占位
}

/// A 看黑板：仅当 DONE=1 才 ready，并取 [15:0] 作为 result。
fn a_poll(ctrl: u32) -> (bool, u32) {
    // TODO: ready = DONE 位是否为 1；result = 低 16 位。
    //   // TODO[a] 一次性解包：(ctrl & DONE != 0, ctrl & RESULT_MASK)
    //   // ELSE[b] 两步式：先判 DONE，再取 RESULT
    (false, 0) // ← 占位
}
...
fn try_lock(lock: &mut u32) -> bool {
    // TODO: 调 tas(*lock) 得 (newv, got)，把 newv 写回 *lock，返回 got。
    //   // TODO[a] 基于 tas 自旋（本题一拍即可）
    //   // ELSE[b] 基于 cas(cur,exp,new)：cur==exp 才写 new
    false // ← 占位
}
...
fn b_step(ctrl: u32, job: u32) -> u32 {
    // TODO: 若 ctrl 的 START 位为 1，返回 DONE | (job & RESULT_MASK)（同时清掉了 START）；
    //       否则原样返回 ctrl。
    ctrl // ← 占位
}
...
fn a_step(ctrl: u32, phase: u32) -> (u32, u32, u32) {
    // TODO:
    //   phase==0: 置 START，相位进 1，post=0           → (ctrl | START, 1, 0)
    //   phase==1 且 DONE=1: post=result*2，清 DONE，相位回 0
    //   phase==1 且 DONE=0: 原地等                       → (ctrl, 1, 0)
