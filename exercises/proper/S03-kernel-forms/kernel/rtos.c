/* S03 · form 2: a minimal cooperative RTOS (STUDENT version -- fill the TODOs).
 *
 ...
 *   (1) rtos_yield()  -- hand the CPU back to the scheduler
 *   (2) rtos_run()    -- the round-robin dispatch over the static task table
 * Everything else (context layout, __switch in switch.S, task creation,
 * bootstrap/exit, the demo workload) is given.
 */
#include "kernel.h"
...
void rtos_yield(void) {
    /* TODO(1): switch from the current task back to the scheduler.
     *   __switch(&task_ctx[current], &sched_ctx);
     * For now this stub does nothing, so tasks never get to run cooperatively. */
}
...
/* round-robin scheduler over the static task table. */
static void rtos_run(void) {
    /* TODO(2): keep switching into READY tasks until all have exited.
     * Sketch:
     *   for (;;) {
     *       int alive = 0;
     ...
     *       if (alive == 0) break;
     *   }
     * The stub below runs no task, so RTOS_PASS will not be printed yet. */
    kputs("[rtos] scheduler not implemented yet (TODO)\n");
}
