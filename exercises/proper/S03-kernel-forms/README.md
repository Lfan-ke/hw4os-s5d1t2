# 正经·S03 · 内核形态认知（unikernel / RTOS / exokernel）

> 承接 S01（已能引导进 S 态）。本课不碰新硬件，而是**在同一个 S 态内核里**演示三种经典「内核形态」的心智模型，并亲手补完一个最小协作式 RTOS。
...
```
labctl run proper/S03-kernel-forms
...
- 协作式 → 抢占式：接上 S02 的时钟中断，在 trap 里触发 `__switch` 即得分时（见 S05/S08）。
- exokernel 的「安全多路复用」推到极致就是 unikernel-on-exokernel：libOS 把 OS 抽象全搬到用户态。
- 把 `yield` 换成 `await` + 状态机，就是 S04 的无栈协程 / async runtime。
