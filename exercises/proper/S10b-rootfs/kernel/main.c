...
/* =========================================================================
 * 填空点 2：在 RAM-fs 里找到 /init，把它读进可执行缓冲，并在 U 态交棒运行。
 *   1) int ino = fs_lookup("init")；找不到（ino<0）就 return（USERSPACE 不会通过）。
 *   2) int n = fs_read((uint32_t)ino, init_exec, sizeof init_exec)；n<=0 就 return。
...
static void load_and_run_init(void) {
    (void)init_exec;
    (void)user_stack;
    /* TODO: 定位 /init、读进 init_exec、run_user 交棒（见上）。
     * 占位：什么都不做 —— /init 不会运行、进程不退出，USERSPACE 出不来。 */
}
