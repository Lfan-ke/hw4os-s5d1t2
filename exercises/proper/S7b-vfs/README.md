# 正经·S7b · VFS：内核虚拟文件系统层（vtable 多态 + 挂载表）

> 承接 S7（单一具体 RAM-fs）。S7 里 `fs_open/fs_read` 直接捅到那一个 FS 的实现；本课在它之上
...
2. **两个具体 FS**（给定）：`ramfs.c`（内存 inode/目录，复用 S7 思路，预置 3 个文件）挂 `"/"`；
...
- 把 S7 的块设备 RAM-fs 接成一个 `vfs_ops` 挂上来，与 devfs 并存；
