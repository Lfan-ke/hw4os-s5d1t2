 ...
 *
 * 承接 S13（多核启动）与 S5（调度器）。单镜像、按 hartid 分支：
 ...
 *
 * TODO[角色分派表]：填入每颗 hart 的角色（ROLE_BIG / ROLE_LITTLE）。
 *   - hart0 跑调度器，必须是 ROLE_BIG；整机至少 1 大核 + 1 小核。
 *   - 参考拓扑：{ ROLE_BIG, ROLE_BIG, ROLE_LITTLE, ROLE_LITTLE }
 * 现为占位（全 ROLE_NONE）：能编译、能跑，但 ROLE_PASS / AFFINITY_PASS 不会通过。
 */
static int cpu_role[NCORE] = { ROLE_NONE, ROLE_NONE, ROLE_NONE, ROLE_NONE };
