# 正经·S5b · 内核堆分配器（rcore ch4 的 kernel heap）

> 承接 S3（内核已能跑 C、有静态数据）。到目前为止内核里所有东西都是**静态/栈上**的：
...
- **接 Rust `global_allocator`（对照 S1b）**：把 `kalloc/kfree` 包成 `GlobalAlloc`，
