/* S06e · 内核入口 / 测试驱动（给定脚手架；唯一留白是「给 target 发 IPI」那一行）。
 *
 * 三段：
 *   1) IPI：引导核用 SBI IPI 给 target 发软件中断（OpenSBI 落到 target 的 CLINT MSIP），
 *      target 的 trap 处理累加 ipi_count - IPI_PASS。  ← 本文件留白：把 IPI 发出去。
 *   2) CLAIM：UART 源同时使能到两核 S-context，回环自激置 pending，两核在 barrier 处一起
 *      读 claim，PLIC 仲裁出恰一个赢家、一个输家 - CLAIM_PASS。  ← claim 在 plic.c 留白。
...
void kmain(void) {
    ...
    smp_fence();
    /* TODO: 给 target 发一次 IPI（S 态软件中断）。
     *   提示：sbi_send_ipi(1UL, target);
     *   它经 OpenSBI 写 target 的 CLINT MSIP、把 mip.SSIP 反射给 S 态 → 目标取到 scause=1。
     *   不发则 target 永不进软件中断处理，ipi_count 恒为 0（IPI_MISS、无 IPI_PASS）。 */
