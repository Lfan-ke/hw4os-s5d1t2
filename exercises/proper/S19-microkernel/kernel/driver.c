 ...
 * 你要实现的：driver_register（找空槽 + 存名字与函数指针）。
 * 注册表查找、字符串比较、收尾测试均给定。 */
#include "kernel.h"
#include "driver.h"
...
int driver_register(const char *name, drv_read_fn fn) {
    /* TODO:
     *   遍历 g_drv[0..DRV_MAX)，找第一个 used==0 的空槽：
     *     置 used=1；s_cpy(g_drv[i].name, name) 登记名字；g_drv[i].read = fn 登记操作；
     *     返回槽号 i。
     *   全满则返回 -1。 */
    (void)name; (void)fn;
    return -1;   /* 占位：未注册任何驱动 → 派发找不到 → DRIVER_SYS 不达成（不崩溃） */
}
