# 正经·S5d · 内核阻塞同步原语：互斥锁 / 信号量 / 条件变量

> 承接 S5（协作式 `__switch` 任务）与 S14（协作任务上的 IPC）。本课在「协作任务运行时」之上，
...
本课用 S5 的 `__switch` 搭一个最小协作运行时，给每个任务加 `READY / RUNNING / BLOCKED / EXITED` 态，并提供两个调度原语：
...
- **抢占式**：接 S2 时钟中断，让阻塞/唤醒在任意指令处也安全——需要在临界区关中断或用原子操作保护原语自身的元数据。
