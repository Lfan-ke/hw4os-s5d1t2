# 正经·S06b · 硬件抽象层(HAL)：迷你 Abstract Machine

> 承接 S02（trap）/ S05（上下文切换）/ S06（裸 MMIO 驱动）。本课换一个视角：把“裸机硬件细节”
> 抽象成一组**可移植 HAL API**——程序只调 HAL，不碰具体寄存器，于是同一份代码能跑遍不同硬件。
> 这正是 YSYX **Abstract Machine** 的核心思想，我们在 S 态把它最小复刻一遍。

## 0. 这节课在讲什么

S06 的驱动直接碰 MMIO 寄存器、S02 的 trap 直接写 `stvec/sepc`——程序和**具体硬件**绑死了。
...
在它之上，我们仿 `yield-os` 造 **2 个任务互相 `yield`**，调度器轮转——和 S05 的协作式调度同构，
...
- 补 **VME（虚存）**：`map/protect/ucontext`，把任务送进 U 态独立地址空间（接 S05c 分页）。
- 补 **MPE（多核）**：`cpu_count/atomic_xchg`，多核各跑一份调度（接 S13/S15）。
- 换一个 **arch** 实现同一套 `am.h`（如改用 `ecall`+`medeleg`、或 M 态版本），`main.c` 不变。
- 把已有 **S02/S05** 重构到 CTE 之上：trap 与调度都只跟 `Event/Context` 打交道。
