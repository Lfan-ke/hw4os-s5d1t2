 ...
 *
 * 你要实现的就是下面两个核心：wq_block / wq_wake_one（共两处 TODO）。
 * 其余外壳（mutex/sem/condvar 的 lock/unlock 等）与三个测试均已给定。
 *
 * 占位实现用 sync_yield 自旋让出代替“真阻塞”：程序能编译、能跑、数据甚至能搬对，
 * 但没有任何任务进入 BLOCKED 态（g_block_events 恒为 0）→ 判据看不到“真阻塞”，
 * 不会打印 *_PASS / ALL_PASS。把两处 TODO 填对即可。 */
#include "kernel.h"
#include "sched.h"
#include "sync.h"
...
static void wq_block(struct WaitQueue *wq) {
    /* TODO（阻塞核心）：
     *   1) 把 cur_task() 入队到 wq：wq->ids[wq->tail] = cur_task();
     *      wq->tail = (wq->tail + 1) % WAIT_CAP; wq->count++;
     *   2) sched_block();   // 置 BLOCKED + __switch 走人；被唤醒后从这里返回
     * 占位：不入队、不阻塞，仅自旋让出 —— 能跑、不卡死，但“没有真正阻塞”，
     *   g_block_events 始终为 0 → 不会 PASS。 */
    (void)wq;
    sync_yield();
}
...
static int wq_wake_one(struct WaitQueue *wq) {
    /* TODO（唤醒核心）：
     *   若 wq->count == 0 返回 -1；
     *   否则 id = wq->ids[wq->head]; wq->head = (wq->head + 1) % WAIT_CAP; wq->count--;
     *        sched_wake(id);   // 置回 READY
     *        返回 id。
     * 占位：什么都不唤醒（配合上面的自旋占位，程序仍靠 yield 推进，但不算真阻塞）。 */
    (void)wq;
    return -1;
}
