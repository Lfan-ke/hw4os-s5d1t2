# 正经·S09 · 极简 libc（给 U 态用户程序用）

> 承接 S08：内核已能 `sret` 跌入 U 态、用户经 `ecall` 求服务、`exit` 后被回收。
> 但 S08 的用户程序是「裸手写 ecall」。本课在 U 态之上盖一层最小 **libc**——
...
- **真正的进程隔离**：本实验用户与内核同地址空间（同 S08），libc 与内核链在同一个 ELF；
