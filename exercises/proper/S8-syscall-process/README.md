# 正经·S8 · 用户态与系统调用（rcore ch2 批处理风）

> 承接 S2（trap 入口/分发已就绪）。本课让内核第一次**跌入 U 态**跑一个用户程序，
...
（填空版占位：trap_handler 只前移 sepc、不分发 → 进程退不出、内核回收不到，**跑不出 ALL_PASS**。）

```
labctl run proper/S8-syscall-process
...
- **进程模型**：`fork` / `exec`（ELF 加载）/ `wait`，多道程序与就绪队列（接 S5 调度器）。
- **文件类 syscall**：`open/read/write/close/dup` 接 VFS（S7）。
