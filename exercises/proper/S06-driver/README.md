# 正经·S06 · 裸机 MMIO 驱动 + 设备树

> 承接 S02（已能进 S 态、处理 trap）。本课不靠 SBI 代劳，而是**直接读写外设寄存器**驱动一个串口；再用 `dtc` 把设备树编成 dtb，内核解析它发现设备，最后用 **compatible 字符串**在驱动表里匹配并 `probe`——这正是 Linux「平台总线」的最小骨架。
...
- 设备树还携带 `interrupts`、`clocks`、子节点；S07 会用 RAM 盘（接口同 virtio-blk MMIO）做块设备读写。
