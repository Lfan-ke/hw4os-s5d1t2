/* S06 · 平台总线简化版：驱动表 + compatible 字符匹配 + probe（学生填空）。
 * 现代内核里设备由设备树/ACPI 提供 compatible 字符串，内核拿它去驱动表里找 probe。 */
#include "dev.h"

/* TODO(3): compatible 字符串比较 —— 相等返回 0（仿 strcmp 语义）。
 *   提示：
 *     while (*a && *a == *b) { a++; b++; }
 *     return (int)(unsigned char)*a - (int)(unsigned char)*b;
 * 现在的占位恒返回 1（永远“不相等”），于是没有驱动会被匹配，PROBE_PASS 出不来。 */
static int k_strcmp(const char *a, const char *b) {
    (void)a; (void)b;
    return 1; /* TODO: 实现真正的字符串比较 */
}
...
int driver_match_and_probe(const char *compatible, uint64_t base) {
    ...
    for (int i = 0; i < DRIVER_COUNT; i++) {
        if (k_strcmp(driver_table[i].compatible, compatible) == 0) {
            if (!driver_table[i].probe) return 0;
