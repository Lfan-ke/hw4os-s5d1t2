# 正经·S8b · mmap 与按需调页（demand paging，建在 S5c 之上）

> S5c 是「开机就把页全映射好」。本实验反过来：`mmap` 时**什么都不分配**，只在一张 VMA 表里
> 登记「这段 VA 将来合法」；等程序**真的访问**到某一页，硬件缺页，内核才临时给那一页补一个
> 物理帧、映进页表、让出错指令重跑一遍成功。缺页在这里不是错误，而是 OS 介入补页的**机制**。

## 0. 这节课在讲什么

S5c 里我们手动 `map_one` 把每个虚拟页都映射到物理帧——**预先全映射 (eager)**。但真实系统里
...
- **文件映射 `mmap(fd,...)`**：VMA 记下 `{文件, 偏移}`；缺页时不是给零帧，而是从 VFS(S7) 读入该页。
...
- 对照 **S5c（预先全映射）**：S5c 开机 `map_one` 把区间每页都映了；这里 mmap 只登记、页表留空，
