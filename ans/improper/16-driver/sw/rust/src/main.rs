...
fn driver_probe(dev: &MmioDevice) -> bool {
    mmio_read(dev, REG_ID) == DEV_MAGIC
}

/// 初始化握手 + 突发收发：① 置 CTRL 使能 ② 轮询 STATUS.ready ③ 逐字节写 DATA。
fn driver_io(dev: &mut MmioDevice, msg: &[u8]) {
    // TODO[a] 忙等轮询 STATUS 单字节握手（本参考解）：
    mmio_write(dev, REG_CTRL, CTRL_ENABLE);
    let mut spun = 0;
    while mmio_read(dev, REG_STATUS) & STATUS_READY == 0 {
        spun += 1;
        if spun > 10_000 {
            break; // 防呆：真硬件 ready 不来要超时，别死等
        }
    }
    for &b in msg {
        mmio_write(dev, REG_DATA, b as u32);
    }
...
fn board_nodes() -> Vec<DevNode> {
    vec![
        ...
        DevNode { name: "blink@10001000".into(), compatible: "acme,blink".into(), base: 0x1000_1000, size: 0x1000 },
        // gpio@10002000：compatible="acme,gpio"，reg=<0x10002000 0x1000>
        DevNode { name: "gpio@10002000".into(), compatible: "acme,gpio".into(), base: 0x1000_2000, size: 0x1000 },
        // lamp@10003000（给定）：16.2 时是未知 compatible，16.3 注册 blink-v2 后被发现。
...
fn match_and_probe(nodes: &[DevNode], drivers: &[Driver]) -> (usize, Vec<String>) {
    ...
    for n in nodes {
        let mut hit = false;
        for d in drivers {
            if d.compatible == n.compatible {
                let _inst = (d.probe)(n.base, n.size);
                println!("PROBE {}@{:x}", d.name, n.base);
                matched.push(format!("{}@{:x}", d.name, n.base));
                hit = true;
                break;
            }
        }
        if !hit {
            skipped += 1; // 未知 compatible：fallback 跳过
        }
    }
    (skipped, matched)
}
...
fn all_drivers() -> Vec<Driver> {
    vec![
        D_BLINK,
        D_GPIO,
        // 16.3 新增的“这一行登记”：让 acme,blink-v2 被自动发现并 probe。
    ...
    for n in nodes {
        for d in drivers {
            if d.compatible == n.compatible {
                ...
                let idx = counter.entry(d.name).or_insert(0);
                let path = format!("/dev/{}{}", d.name, *idx);
                *idx += 1;
                bus.devs.push((path, inst));
                break;
            }
        }
    }
...
fn user_read(bus: &Bus, path: &str) -> Option<u32> {
    bus.devs.iter().find(|(p, _)| p == path).map(|(_, f)| f.read())
}
/// 用户态 write：转发到设备 ops。
fn user_write(bus: &mut Bus, path: &str, v: u32) -> bool {
    if let Some((_, f)) = bus.devs.iter_mut().find(|(p, _)| p == path) {
        f.write(v);
        true
    } else {
        false
