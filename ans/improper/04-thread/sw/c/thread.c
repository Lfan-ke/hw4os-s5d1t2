...
static void ctx_save(const Vcpu *vcpu, Context *cur) {
    /* ELSE[b]：整体赋值（结构体含 gpr 数组 + 三个 CSR）。 */
    *cur = vcpu->regs;
}
...
static void spawn_shared_pair(Process *p, Process *other, Tcb *t1, Tcb *t2) {
    ...
    t2->tid = 2; ctx_zero(&t2->ctx); t2->proc = p;
}
...
static int check_sched(void) {
    ...
    for (int k = 0; k < 3; k++) {
        ...
        ctxs[k].gpr[0] = 0x1000 + (uint64_t)k; /* 独立标记 */
        tids[k] = (uint32_t)(k + 1);
    }
