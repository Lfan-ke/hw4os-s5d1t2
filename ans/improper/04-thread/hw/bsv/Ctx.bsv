// 双上下文寄存器文件 —— 硬件路径（BlueSpec SV 参考解）。
// 两套上下文 = 两组复制的寄存器（影子寄存器组）；vcpu_read/vcpu_sum 是共享后端：
// 由 active 一拍选中一套上下文。与软件 / Verilog 变体输出逐项一致。
package Ctx;
...
module mkTbCtx (Empty);
   ...
   rule run (!done);
      ...
      g = 0;
      let h0 <- chkEq("SHARE", vcpu_sum(c0, c1, 0, 0, 1), 300); g = g + h0; // 100+200
      let h1 <- chkEq("SHARE", vcpu_sum(c0, c1, 1, 0, 1), 33);  g = g + h1; // 11+22
