// 16e - interrupt aggregation + multi-core arbitration -- device side (BlueSpec SV, reference).
...
module mkIntcAgg (IntcAgg);
   ...
   // ── claim = atomic gateway take: only when pending and gateway free does this hart win ──
   method ActionValue#(Word) claim(Hart hart);
      Word id = 0;
      if (pending && !inflight) begin
         id       = irqId;
         inflight <= True;      // gateway enters in-flight
         wonR     <= True;
         winnerR  <= hart;      // record the unique handler
      end
      return id;
   endmethod
