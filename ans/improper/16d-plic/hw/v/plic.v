...
module plic (
    ...
    wire elig1 = pending[1] & enable[1] & (prio1 > threshold);
    wire elig2 = pending[2] & enable[2] & (prio2 > threshold);
    wire elig3 = pending[3] & enable[3] & (prio3 > threshold);
    wire elig4 = pending[4] & enable[4] & (prio4 > threshold);

    // ── 学生填②：优先级仲裁 - 取最高 prio；同 prio 取最小 id（自高 id 向低用 >= 让低 id 胜）──
    reg [2:0] bid;
    reg [1:0] bp;
    always @(*) begin
        bid = 3'd0; bp = 2'd0;
