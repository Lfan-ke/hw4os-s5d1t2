/* ISA 模拟器：CPU 不过是一个「取指 → 译码 → 执行」的循环 —— C 参考解。
 *
 * 母题：一颗 CPU 的本质，就是反复地
...
static int step(Cpu *c) {
    ...
    case 0x33: /* OP: R 型 add/sub */
        if (d.funct3 == 0) {
            ...
            if (d.funct7 == 0x00)
                wreg(c, d.rd, c->regs[d.rs1] + c->regs[d.rs2]);
    ...
    case 0x63: { /* BRANCH: beq/bne */
         ...
         * 目标 = pc + imm_b（相对 pc 的有符号偏移）；条件成立才改 npc。 */
        int take = 0;
        if (d.funct3 == 0)
            take = (c->regs[d.rs1] == c->regs[d.rs2]);
        else if (d.funct3 == 1)
            take = (c->regs[d.rs1] != c->regs[d.rs2]);
        if (take)
            npc = c->pc + (uint64_t)d.imm_b;
        /* ────────────────────────────────────────── */
        break;
    }
    case 0x6f: /* JAL（给定） */
...
static int check_difftest(void) {
     ...
     * 全程一致则打印 DIFFTEST_PASS 并 return 1。列名用 col_name。 */
    char nm[8];
    for (int k = 0; k < GOLDEN_N; k++) {
        for (int col = 0; col < SNAP; col++) {
            if (dut[k][col] != GOLDEN[k][col]) {
                col_name(col, nm);
                printf("DIFF_BAD 第 %d 条指令 %s: ref=%llu dut=%llu\n", k + 1, nm,
                       (unsigned long long)GOLDEN[k][col], (unsigned long long)dut[k][col]);
                return 0;
            }
        }
    }
    ...
    printf("DIFFTEST_PASS 全 %d 步逐指令与黄金参考一致\n", GOLDEN_N);
    return 1;
}
