...
function Bit#(64) ipc_op(Bit#(4) op, Bit#(32) a, Bit#(32) b);
   ...
   Bit#(64) y = 0;
   case (op)
      4'd0: y[31:0] = bf;                                         // BFINISH
      4'd1: begin y[15:0] = a[15:0]; y[32] = ready; end           // APOLL
      4'd2: begin y[0] = 1'b1; y[1] = got; end                    // TAS
      4'd3: y = 0;                                                // UNLOCK
      4'd4: begin y[7:0] = cdec; y[8] = ~cdec[7]; end             // DOWN: ok=(count'>=0)
      4'd5: y[7:0] = cinc;                                        // UP
      4'd6: begin                                                 // ASTEP
               if (b[0] == 0) begin
                  y[31:0] = a | cSTART;     // phase0: 按门铃
                  y[48]   = 1'b1;           // phase'=1
               end else if (ready == 1) begin
                  y[31:0]  = a & ~cDONE;    // 见 DONE: 取数算后续、清 DONE
                  y[47:32] = post16;
                  y[48]    = 1'b0;          // phase'=0
               end else begin
                  y[31:0] = a;              // B 没干完: 死等
