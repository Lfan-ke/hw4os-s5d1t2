...
static unsigned long lookup(const Library *lib, const char *sym) {
    ...
    for (int i = 0; i < lib->nsym; i++)
        if (strcmp(lib->syms[i].name, sym) == 0)
            return lib->base + lib->syms[i].off;
    return 0;
}
...
static DynProgram make_dyn(const char *needed, const char *const *syms, int nsym) {
    ...
    p.own = OWN_CODE;
    strncpy(p.needed, needed, sizeof(p.needed) - 1);
    p.needed[sizeof(p.needed) - 1] = 0;
    for (int i = 0; i < nsym; i++) {
        strncpy(p.got[i].sym, syms[i], sizeof(p.got[i].sym) - 1);
        p.got[i].sym[sizeof(p.got[i].sym) - 1] = 0;
        p.got[i].resolved = 0;
    }
    p.ngot = nsym;
    return p;
...
static void resolve(DynProgram *prog, const Library *lib) {
    ...
    for (int i = 0; i < prog->ngot; i++)
        prog->got[i].resolved = lookup(lib, prog->got[i].sym);
}
...
static unsigned long dyn_total_space(unsigned long n, unsigned long n_syms) {
    ...
    return LIB_CODE + n * (OWN_CODE + n_syms * GOT_SLOT);
}
...
static int lazy_bind(DynProgram *prog, const Library *lib, const char *const *used, int nused) {
    ...
    for (int u = 0; u < nused; u++)
        for (int i = 0; i < prog->ngot; i++)
            if (strcmp(prog->got[i].sym, used[u]) == 0 && prog->got[i].resolved == 0) {
                prog->got[i].resolved = lookup(lib, used[u]);
                c++;
            }
    return c;
}
...
static unsigned long dlsym_addr(const Registry *reg, const char *libname, const char *sym, int *err) {
    ...
    for (int i = 0; i < reg->n; i++)
        if (strcmp(reg->libs[i].name, libname) == 0) {
            unsigned long a = lookup(&reg->libs[i], sym);
            if (a == 0) { *err = 1; return 0; }
            *err = 0;
            return a;
        }
    *err = 1;
