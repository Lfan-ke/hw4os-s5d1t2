// 16c · core-local interrupt - device side (BlueSpec SV, reference).
...
// Fixed scenario: PERIOD=5, NTICK=16 -> timer fires 3 times (mtime=5/10/15); software int raised twice.
package Clint;
...
module mkClint (Clint);
   ...
   method Bool    mtip  = (mtimeR >= mtimecmpR);   // comparator
   method Bit#(1) msip  = msipR;
endmodule
