//! 容器隔离：namespace 切视图 + cgroup 卡配额 —— Rust 参考解。
//!
...
fn ns_local_pid(globals: &[u32], global: u32) -> Option<u32> {
    for (i, &g) in globals.iter().enumerate() {
        if g == global {
            return Some(i as u32 + 1); // 下标 i → 局部 pid i+1（init=globals[0]→1）
        }
    }
    None
}
...
fn ns_global_pid(globals: &[u32], local: u32) -> Option<u32> {
    if local >= 1 && (local as usize) <= globals.len() {
        Some(globals[local as usize - 1])
    } else {
        None
    }
}
