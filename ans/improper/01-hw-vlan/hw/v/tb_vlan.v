// 01-hw-vlan Verilog testbench（给定，学生勿改）。
...
module tb_vlan;
    ...
    initial begin
        ...
        chk(mk(1'b0, 1'b1, 6'd10, 16'h1234), 32'h8000_1234);                 // 有Tag→剥离
        chk(mk(1'b0, 1'b0, 6'd0,  16'h1234), 32'hC005_1234);                 // 无Tag→插PVID=5
        chk(mk(1'b1, 1'b1, 6'd10, 16'h1234), 32'h8000_1234);                 // 发包→剥离
        if (gerr == 0) $display("ACCESS_PASS");

        // ── TRUNK, allow={10,20} ──
        mode = 2'd1; pvid = 6'd0; allow = (64'd1 << 10) | (64'd1 << 20); untag = 64'd0; gerr = 0;
        chk(mk(1'b0, 1'b1, 6'd10, 16'hABCD), 32'hC00A_ABCD);                 // 允许→通过
        chk(mk(1'b0, 1'b1, 6'd30, 16'h1111), 32'hA000_0000);                 // 不允许→丢弃
        chk(mk(1'b0, 1'b0, 6'd0,  16'h2222), 32'hA000_0000);                 // 无Tag→丢弃
        chk(mk(1'b1, 1'b1, 6'd10, 16'hABCD), 32'hC00A_ABCD);                 // 发包→保留
        if (gerr == 0) $display("TRUNK_PASS");

        // ── HYBRID, allow={10,20}, untag={10} ──
        mode = 2'd2; pvid = 6'd0; allow = (64'd1 << 10) | (64'd1 << 20); untag = (64'd1 << 10); gerr = 0;
        chk(mk(1'b0, 1'b1, 6'd20, 16'h0F0F), 32'hC014_0F0F);                 // 允许→通过
        chk(mk(1'b0, 1'b1, 6'd30, 16'h3333), 32'hA000_0000);                 // 不允许→丢弃
        chk(mk(1'b1, 1'b1, 6'd10, 16'h0F0F), 32'h8000_0F0F);                 // 发包 untag[10]→剥离
        chk(mk(1'b1, 1'b1, 6'd20, 16'h0F0F), 32'hC014_0F0F);                 // 发包 untag[20]=0→保留
