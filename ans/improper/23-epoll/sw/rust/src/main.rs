//! I/O 多路复用：select / epoll / 边沿触发 / O(ready) 伸缩性 —— Rust 参考解。
//!
...
//!                 用计数器证明。
//!
//! 你只需填 2 处：epoll_wait 的「只收集就绪兴趣 fd」与 ET 的「边沿判定」。
...
fn epoll_wait(ep: &mut Epoll, fds: &[Fd]) -> Vec<usize> {
    ...
    for fd in pending {
        ...
        // ET 不武装：报告完即摘除，要等下一次上升沿(edge_ready)才再上链。
