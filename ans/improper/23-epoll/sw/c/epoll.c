/* I/O 多路复用：select / epoll / 边沿触发 / O(ready) 伸缩性 —— C 参考解。
 *
 ...
 *
 * 四段：1 SELECT 扫描全部找就绪集；2 EPOLL 只返回已注册且就绪、未注册的不返回；
 *       3 EDGE 边沿触发 ET 只通知一次 vs 水平触发 LT 持续通知；
 *       4 SCALE 1 就绪/1000 fd，epoll O(ready) vs select O(n) 计数器证明。
 *
 * 学生只需填 2 处：epoll_wait 的「只收集就绪兴趣 fd」与 ET 的「边沿判定」。
 * 下方测试 harness 勿改。
 */
#include <stdio.h>
#include <string.h>
...
static int epoll_wait_op(Epoll *ep, Fd *fds, int *out) {
    ...
    for (int i = 0; i < plen; i++) {
        ...
        /* ← 学生填这一处：只收集 registered && readable 的 fd */
        if (registered && readable) {
            out[olen++] = fd;
        }

        /* LT 重新武装：仍可读就挂回；ET 不武装(等下次上升沿)。 */
