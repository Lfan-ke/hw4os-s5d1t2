/* 发行版与根文件系统：从光秃秃的内核到能进 shell 的 rootfs —— C 参考解。
 *
...
static void busybox_main(int argc, const char **argv, char *out, size_t cap) {
    ...
    if (strcmp(cmd, "echo") == 0) {
        join_args(out, cap, argc, argv, " ", 0);
    } else if (strcmp(cmd, "ls") == 0) {
        join_args(out, cap, argc, argv, "\n", 1);
    } else if (strcmp(cmd, "cat") == 0) {
        join_args(out, cap, argc, argv, "\n", 0);
    } else if (strcmp(cmd, "mount") == 0) {
        char tmp[120];
        join_args(tmp, sizeof tmp, argc, argv, " ", 0);
        snprintf(out, cap, "mounted %s", tmp);
    } else {
        snprintf(out, cap, "busybox: applet not found: %s", cmd);
    }
}
...
static void mock_init(Tree *t, Log *log) {
    ...
    buf[bl] = 0;
    /* 逐行解析（手写，不依赖 strtok），每行形如 ::action:command */
    size_t i = 0;
    while (i < bl) {
        size_t j = i;
        while (j < bl && buf[j] != '\n') j++;
...
static int cpio_unpack(const unsigned char *arc, size_t len, Tree *out) {
    ...
    size_t off = 0;
    while (1) {
        if (off + CPIO_HDR > len) return -1;
        if (memcmp(arc + off, CPIO_MAGIC, 4) != 0) return -1;
        char type = (char)arc[off + 4];
        uint32_t namelen = r32(arc + off + 5);
        uint32_t bodylen = r32(arc + off + 9);
        size_t nstart = off + CPIO_HDR;
        size_t nend = nstart + namelen;
        size_t bend = nend + bodylen;
        if (bend > len) return -1;
        char name[PATH_CAP];
        size_t nl = namelen;
        if (nl >= PATH_CAP) nl = PATH_CAP - 1;
        memcpy(name, arc + nstart, nl);
        name[nl] = 0;
        if (strcmp(name, CPIO_TRAILER) == 0) break;
        tree_set(out, type, name, arc + nend, bodylen);
        off = bend;
    }
    return 0;
}
