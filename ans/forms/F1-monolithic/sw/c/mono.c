/* 形态 F1 · 宏内核 / 单内核（monolithic）—— C 参考解。
 *
...
static void syscall_dispatch(uint32_t inode, const uint32_t *prios, int n,
    ...
    uint32_t data = fs_read(inode);              /* 调用 1：fs 服务 */
    uint32_t idx = (uint32_t)sched_pick(prios, n); /* 调用 2：调度服务 */
    uint32_t ack = driver_io(CMD_WRITE, data);   /* 调用 3：驱动服务 */
    *result = data + idx + ack;
    *hops = 3u;
}
...
static int detect_corruption(const uint32_t *region, const uint32_t *baseline, int n) {
    return memcmp(region, baseline, (size_t)n * sizeof(uint32_t)) != 0;
}
