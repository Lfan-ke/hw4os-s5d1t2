//! 形态 F1 · 宏内核 / 单内核（monolithic）—— Rust 参考解。
//!
//! 本质：fs / 调度 / 驱动 / 内存 等所有服务都跑在**同一个地址空间**里，
//! 子系统之间靠**直接函数调用**协作——没有 IPC、没有特权切换。
//!   好处：快（一次系统调用 = 几次普通函数调用，零消息、零上下文切换）。
//!   代价：脆（没有隔离边界——一个驱动越界写，就能直接踩坏调度器的内存）。
//! 真实例：xv6、Linux、FreeBSD——fs/sched/driver 全在 kernel/ 下共享 struct。
//!
//! 两段 demo：
//!   1. MONO  —— 把 fs_read / sched_pick / driver_io 当「内核服务」直接串起来
//!               (syscall_dispatch)：结果正确、IPC=0、只是几次函数调用。
//!   2. FRAGILE —— 同一块「内核地址空间」(kmem 数组)里，驱动缓冲区紧挨调度器
//!               就绪队列。一次越界 DMA 写就污染了调度器数据，整个「内核」
//!               的调度决策随之出错——我们检测到了这次跨子系统的破坏
//!               (说明根本没有隔离边界)。
//!
//! 学生只需填 6 个函数体；下方测试 harness（向量 + 不变量 + PASS 打印）勿改。
#![allow(dead_code)]

// ── 单一内核地址空间布局 ─────────────────────────────────────────
// 一块物理上连续的「内核内存」kmem[8]，逻辑上切给两个子系统：
//   [0..4) 驱动 TX 缓冲区     (DRV_BASE=0, DRV_LEN=4)
//   [4..8) 调度器就绪队列优先级 (SCHED_BASE=4, SCHED_LEN=4)
// 注意：这条边界**只存在于注释里**，硬件层面它就是一根连续数组——
// 这正是宏内核「脆」的根源。
const KMEM: usize = 8;
...
fn syscall_dispatch(inode: u32, prios: &[u32]) -> (u32, u32) {
    // 直接调用链：在同一地址空间里，服务就是普通函数，一个接一个调。
    let data = fs_read(inode); // 调用 1：fs 服务
    let idx = sched_pick(prios) as u32; // 调用 2：调度服务
    let ack = driver_io(CMD_WRITE, data); // 调用 3：驱动服务
    (data + idx + ack, 3)
}
