//! 形态认知 · F2 微内核（microkernel）—— Rust 参考解。
//!
...
//!
//! 你只需填两个函数：`cap_check`（能力校验）与 `dispatch`（消息分发）。
...
fn cap_check(cap: &Cap, world: &World) -> Status {
    if cap.target == SVC_NONE {
        return Status::DenyNoCap;
    }
    if cap.rights & RIGHT_SEND == 0 {
        return Status::DenyNoRight;
    }
    if !world.alive[cap.target] {
        return Status::DenyOffline;
    }
    Status::Ok
}
