//! 形态认知 · F5 框内核(framekernel, Asterinas) —— Rust 参考解。
//!
//! 一句话母题：**宏内核的单地址空间性能 + 微内核式的隔离**，怎么同时拿到？
//! 答案是把可信计算基(TCB)收敛成一个最小「框架(OS Framework / OSTD)」，
//! 框架内部用 `unsafe`/裸指针把底层能力包成**安全 API**；其余「内核子系统」
//! (kernel/ services) 全用安全 Rust，经该 API 访问资源——**隔离来自类型系统，
//! 不是 MMU**。一个子系统拿着别人的句柄也越不过边界改坏别人的状态。
//!
//! 真实原型：Asterinas（蚂蚁 OS Lab，SOSP'25 Best Paper）。
//!   - `ostd/`  = OS Framework，允许 unsafe，把裸操作包成 `Frame`/`VmSpace` 等 safe API。
//!   - `kernel/src/lib.rs:8` = `#![deny(unsafe_code)]` —— 整个服务层禁 unsafe。
//!   - framekernel 的卖点：memory-safety TCB 缩到 OSTD 那一小块、可审计。
//!
//! 本 demo 用最朴素的 host 软件模型把这套权衡演示出来：
//!   1. FRAME    —— 框架给出受控内存区句柄 `Frame`，子系统经安全 API 存取、往返一致。
//!   2. TYPESAFE —— 子系统试图越界改坏邻居：安全 API 类型化拒绝(返回 Err)，邻居毫发无损；
//!                  若改用裸指针写则会破坏——而子系统模块带 `#![forbid(unsafe_code)]`，
//!                  那行裸指针写**根本无法通过编译**（注释里给出会被拒的反例）。
//!   3. MINTCB   —— 审计源码：`unsafe` 只出现在框架 TCB 区段、子系统区段为 0。
//!
//! 学生只需填 2 处：安全 API 的「边界检查」封装，与「unsafe 计数」逻辑。

#![allow(dead_code)]

// ════════════════════════════════════════════════════════════════
// OS Framework（≈ asterinas OSTD）：**唯一**允许 unsafe 的最小可信框架。
// 框架把裸指针能力包成经边界检查的安全 API，对外只暴露 safe 方法。
// ════════════════════════════════════════════════════════════════
mod framework {
    ...
    /// 子系统即便拿到 Frame，也无法越过 len 去改坏邻居——隔离靠类型，不靠 MMU。
    pub struct Frame {
        base: *mut u8,
        len: usize,
    }
...
// （对应 asterinas kernel/src/lib.rs:8 的 `#![deny(unsafe_code)]`）。
// ════════════════════════════════════════════════════════════════
mod subsystems {
    #![forbid(unsafe_code)]
    ...
    ///     //   ^ 编译错误：dereference of raw pointer requires 越权块（已被 forbid 禁止）
...
fn check_typesafe() -> bool {
    ...
    // 隔离不是「全禁」，而是「按类型边界放行」：A 在自己范围内仍可写。
...
fn check_mintcb() -> bool {
    ...
    let tcb = slice_between(src, "LABCTL-TCB-BEGIN", "LABCTL-TCB-END");
    let sub = slice_between(src, "LABCTL-SUBSYS-BEGIN", "LABCTL-SUBSYS-END");
