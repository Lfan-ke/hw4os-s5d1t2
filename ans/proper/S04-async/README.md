# 正经·S04 · embassy 式异步运行时（内核内）

> 承接 S02（trap/timer 已通）。本课在 S 态内核里手搓一个 **embassy 风格的无栈异步运行时**：
...
- **延时 Future**：未到点就返回 `Pending` 并登记到 timer reactor；时钟中断（复用 S02 的
...
## 1. 你要实现的（`kernel/async.c`）

1. **`count_poll`**（状态机推进）：
   ```
   若 state < n：state++; 打标签(label); want = WAKE_NOW; 返回 PENDING
...
3. **`exec_run`**（executor 调度循环）：
   ...
   while 就绪队列非空 或 reactor 非空：
       while 就绪队列非空：
           t = 出队; r = t->poll(t);
           若 r==PENDING：want==WAKE_NOW → 重新入就绪队列；否则 → timer_register(挂 reactor)
           // r==READY：任务完成，丢弃
       若 reactor 非空：wfi(等时钟中断); reactor_wake_due(唤回到期任务)
   ```
   就绪队列原语（`ready_push/pop`、`timer_register`、`reactor_wake_due`）已给定。

`kernel/main.c`（harness，勿改）跑三个场景并判定；`kernel/trap.c`（给定，复用 S02）让时钟中断累加 `g_ticks`。
...
- [ ] 能说清「无让出 → 退化为顺序批处理」，以及 embassy 式 reactor/waker 的角色。

## 4. 引申

- 真实 `Pin`/自引用 Future、`RawWaker` vtable、跨 `await` 借用；多个外设各自的 reactor。
- 把本 executor 接入 S05 调度器（协程任务参与统一调度）；为 S14 异步 IPC 提供运行时底座。
