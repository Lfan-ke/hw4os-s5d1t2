/* S7b · VFS 核心：挂载表 + 路径解析 + vtable 分发（参考解）。
 *
 * 两个学生填空点（见 README）：
...
static struct mount *mount_resolve(const char *path, const char **rel) {
    ...
    for (int i = 0; i < g_nmnt; i++) {
        if (!path_is_prefix(path, g_mnt[i].path)) continue;
        uint32_t L = kstrlen(g_mnt[i].path);
        if (best == 0 || L > bestlen) { best = &g_mnt[i]; bestlen = L; }
    }
    if (!best) return 0;
    const char *p = path + bestlen;     /* 跳过挂载点前缀 */
    while (*p == '/') p++;              /* 再吃掉分隔斜杠 */
    *rel = p;
    return best;
}
