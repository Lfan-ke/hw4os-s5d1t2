/* S06d · 平台总线：驱动表 + compatible 匹配 + probe + bind + /dev 注册（参考解）。
...
/* - 驱动表：设备(compatible) 与驱动(probe) 经字符串解耦匹配 - */
...
/* bind：把已匹配的 driver 绑到 device，并在 /dev 注册一个节点。 */
int driver_bind(struct device *dev) {
    if (!dev || !dev->drv) return 0;
    dev->bound = 1;
    return dev_register(class_to_node(dev->drv->class), dev) >= 0 ? 1 : 0;
}
