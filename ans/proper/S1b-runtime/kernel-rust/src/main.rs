...
unsafe impl GlobalAlloc for Bump {
    unsafe fn alloc(&self, l: Layout) -> *mut u8 {
        let off = &mut *addr_of_mut!(OFF);
        let a = l.align();
        *off = (*off + a - 1) & !(a - 1);
        if *off + l.size() > HEAP_LEN {
            return core::ptr::null_mut();
        }
        let p = (addr_of_mut!(HEAP) as *mut u8).add(*off);
        *off += l.size();
        p
    }
...
#[no_mangle]
extern "C" fn kmain() {
    puts("[S1b-rust] core always-on; alloc needs #[global_allocator]\n");
