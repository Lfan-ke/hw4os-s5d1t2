# 正经·S05e · 进程：fork / exec / wait + 写时复制（rcore ch5）

> 承接 S05c（SV39 分页 / 帧分配 / 缺页捕获）；复用与 S08 相同的 U 态 sret/ecall 机制（本课框架已代办）。
...
```
labctl run proper/S05e-process
...
- **对照 S08**：S08 是 ch2 批处理——单进程、**无分页**、仅特权级隔离；本实验是 ch5——多进程、
