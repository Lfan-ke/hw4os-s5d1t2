/* S10b · cpio(newc) 解包器：把内嵌 initramfs 逐条灌进 RAM-fs（参考解）。
 *
...
/* =========================================================================
 * 学生填空点 1：解析归档中偏移 off 处的一条 newc 头。
 *   - 越界/魔数不符：返回 0（已给守卫，勿删）。
 *   - 读出 namesize(头+94)、filesize(头+54) 两个 8 字符 ASCII 十六进制字段。
 *   - name 紧跟 110B 头之后；data 在 (110+namesize) 补齐到 4B 之后；
 *     next = data 偏移 + filesize 补齐到 4B。
 *   - 若 name == "TRAILER!!!"（归档结束哨兵）：返回 0；否则填好 v 各字段、返回 1。
 * HINT：cpio_hex8 / align4 / kstreq 已给；偏移用「相对 off」算，指针用 arc+绝对偏移。
 * ========================================================================= */
static int cpio_parse_one(const uint8_t *arc, uint32_t len,
    ...
    uint32_t data_off = off + align4(110u + namesize); /* 数据起始（绝对偏移） */
    uint32_t next_off = data_off + align4(filesize);   /* 下一条头（绝对偏移） */

    if (kstreq(name, "TRAILER!!!")) return 0;          /* 收尾哨兵 */
