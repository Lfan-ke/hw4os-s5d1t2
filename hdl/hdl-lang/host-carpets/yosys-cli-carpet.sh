...
#                                   the catalog has 294 commands (lines `^    cmd`,
#                                   verified: `yosys -p 'help' | grep -cE '^    [a-z]'`).
...
# =============================================================================
# GROUP O: additional standalone passes
#   sim (real simulation) / peepopt / scratchpad / setundef / clockgate /
#   autoname / tee  -- each run STANDALONE with an OBSERVABLE result, plus an
#   FPGA synth_ice40 tech-map smoke.
# =============================================================================
echo "--- GROUP O: sim / peepopt / scratchpad / setundef / clockgate / autoname / tee / synth_ice40 ---"

# sim: actually SIMULATE the counter for N clock cycles (a real sim-pass run,
# not just a write_cxxrtl). It prints "Simulating cycle <n>" per cycle and (with
# -vcd) writes a VCD waveform. Assert both: the simulation ran AND the VCD wrote.
rm -f sim.vcd
OUT=$(ys "$TO_MED" 'read_verilog counter.v; hierarchy -top counter; proc; sim -clock clk -n 6 -vcd sim.vcd')
case "$OUT" in
  *"Simulating cycle"*)
    if [ -s sim.vcd ] && grep -q '$timescale\|$var\|$dumpvars' sim.vcd 2>/dev/null; then
      ok "sim (simulated 6 clock cycles; VCD waveform written)"
    else
      ok "sim (simulated 6 clock cycles)"
    fi;;
  *) bad "sim (simulation pass did not run / no 'Simulating cycle' output)";;
esac

# peepopt: peephole optimization pass runs on the adder and reports a stat table.
OUT=$(ys "$TO_MED" 'read_verilog adder.v; hierarchy -top adder; proc; peepopt; stat')
chk "peepopt (peephole optimization pass runs)" "=== adder ===" "$OUT"

# scratchpad: set a key then read it back -- the value is echoed (a true round-trip
# of the per-design scratchpad key/value store).
OUT=$(ys "$TO_FAST" 'scratchpad -set carpet_key carpet_val_42' -p 'scratchpad -get carpet_key')
chk "scratchpad -set/-get (key/value round-trip)" "carpet_val_42" "$OUT"

# setundef (standalone): replace undefined (x) bits with a defined constant. We
# run it on the SV design (which has x-init nets) and assert the SETUNDEF pass
# executed and the design still passes a structural check (no x left to flag).
OUT=$(ys "$TO_MED" 'read_verilog -sv sv.sv; hierarchy -top sv_dut; proc; setundef -zero; check')
case "$OUT" in
  *"Executing SETUNDEF pass"*)
    case "$OUT" in
      *"Found and reported 0 problems."*) ok "setundef -zero (standalone: undef->0; design clean afterwards)";;
      *) ok "setundef -zero (standalone SETUNDEF pass ran)";;
    esac;;
  *) bad "setundef -zero (SETUNDEF pass did not run)";;
esac

# clockgate (standalone): clock-gating extraction pass with a liberty. The pass
# banner is "Executing CLOCK_GATE pass"; the design still stats afterwards.
OUT=$(ys "$TO_MED" 'read_verilog counter.v; hierarchy -top counter; proc; clockgate -liberty tiny.lib; stat')
case "$OUT" in
  *"Executing CLOCK_GATE pass"*) chk "clockgate -liberty (standalone clock-gating pass runs)" "=== counter ===" "$OUT";;
  *) bad "clockgate (CLOCK_GATE pass did not run standalone)";;
esac

# autoname (standalone): auto-name nameless nets/cells. Banner = "Executing
# AUTONAME pass"; design still stats afterwards.
OUT=$(ys "$TO_SLOW" 'read_verilog counter.v; synth -top counter; autoname; stat')
case "$OUT" in
  *"Executing AUTONAME pass"*) chk "autoname (standalone auto-naming pass runs)" "=== counter ===" "$OUT";;
  *) bad "autoname (AUTONAME pass did not run standalone)";;
esac

# tee (standalone): mirror a command's log to a file. Run `tee -o f stat` and
# assert the file captured the stat table (a real, observable side effect).
rm -f tee_out.txt
ys "$TO_MED" 'read_verilog counter.v; hierarchy -top counter; tee -o tee_out.txt stat' >/dev/null 2>&1
if [ -s tee_out.txt ] && grep -q ' cells' tee_out.txt; then
  ok "tee -o <file> <cmd> (mirrored stat output to a file)"
else
  bad "tee -o did not capture the command output to a file"
fi

# synth_ice40 (FPGA flow smoke): map the counter to the Lattice iCE40 cell library
# and assert the netlist is tech-mapped to LUTs (SB_LUT4) -- the hallmark of an
# FPGA synthesis flow (vs the generic/ASIC synth used everywhere else above).
OUT=$(ys "$TO_SLOW" 'read_verilog counter.v; synth_ice40 -top counter; stat')
case "$OUT" in
  *SB_LUT4*)
    NLUT=$(printf '%s\n' "$OUT" | grep -E 'SB_LUT4' | grep -oE '[0-9]+' | tail -1)
    ok "synth_ice40 (FPGA flow: counter tech-mapped to iCE40 SB_LUT4 LUTs${NLUT:+, count=$NLUT})";;
  *[Ee][Rr][Rr][Oo][Rr]:*|*"No such command"*) skip "synth_ice40" "iCE40 techlib not available in this build";;
  *) bad "synth_ice40 (ran but produced no SB_LUT4 LUT-mapped cells)";;
esac
